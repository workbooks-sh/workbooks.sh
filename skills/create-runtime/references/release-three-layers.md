...
## 3. `wbx deploy` (Deploy Kit) — USERS ONLY
...
- **NOT** how *we* publish the compilers package or platform runtime image.
  **Never wire platform-release ops into `wbx deploy`.**

## Rule of thumb

*Compilers* ship as their own ghcr package, published **manually**. The
*runtime* ships via **CI**. **`wbx deploy` is the user's tool to run the
