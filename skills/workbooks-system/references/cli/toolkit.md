# Toolkit commands — `wb toolkit`, `wb compiler`, `wb isolation`
...
**Where these run:** in-process (an agent calling `wb`) they run locally over the
...
## Discover — `wb toolkit list | show | search`
...
## Build — `wb toolkit build <id> [which]`
...
## Self-author — `wb toolkit build-inline`, `wb toolkit promote`
...
## Run a task — `wb toolkit run <id> <task> -- <args…>`
