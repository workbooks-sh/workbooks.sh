...
- **Success:** `valid — <summary>`. **Failure:** `invalid deployment:` + a
  bulleted issue list; `no deployment.org — run wb deploy init`.

## `wb deploy apply [file]`
...
- **Action:** doctor → create the microVM → direct-spawn it in the caller's GUI
  ...
  discovery file.
- **Success:** `local runtime up — http://… (survives app quit; wb deploy down to
...
- **Failure:** `no discovery file — is the daemon up? wb deploy local`;
...
## `wb deploy build` / `wb deploy publish`
...
- **Platform note:** publishing the *runtime image* is normally CI's job, and the
  ...
  conflate `wb deploy` with platform-release ops.
