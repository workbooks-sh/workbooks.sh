...
| `WB_INSTALL_URL` | `https://workbooks.sh/install.sh` | the desktop installer URL (`wb desktop install`) |

## Client target (CLI → running runtime)

| Var | Default | Purpose |
|---|---|---|
| `WB_RUNTIME_URL` | unset | explicit runtime URL for `wb rt`/`wb ctk`/`wb dev`; else the local discovery file |
...
Model-key discovery for `wb dev info` also recognizes these (not `WB_*`):
