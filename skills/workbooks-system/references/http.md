...
| POST | `/rcp/build` | `wb build` — compile a workspace's components → WASM |
...
| GET / POST | `/rcp/store` | `wb stored` (list keys) / `wb store` (archive a workspace) |
...
| POST | `/rcp/toolkit/install` | `wb toolkit push` — install a toolkit directory (zip b64, zip-slip-guarded) |
