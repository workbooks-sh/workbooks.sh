...
WB.view('/authorize', {
  ...
  bare: true,
  async render(el, ctx) {
    // Server load() reads `flow` from the URL and the email from WorkOS `locals.auth`.
    // No client API for the desktop broker exists — reproduce the data here: `flow` from
    // the query string, email from WB.user (the signed-in account). The approve/cancel
    // actions post back to this route's server actions (?/approve · ?/cancel), so we
    // submit a real form to preserve the broker round-trip.
    const params = new URLSearchParams(typeof location !== 'undefined' ? location.search : '');
    const flow = params.get('flow') || (ctx && ctx.params && ctx.params.flow) || '';
    const email = (WB.user && WB.user.email) || '';

    document.title = 'Authorize Workbooks';
...
<div class="authwrap">
  ...
  <form method="POST" class="card">
    ...
    <p class="lede">
      Workbooks for desktop wants to sign in to your account${email ? ` as <strong>${esc(email)}</strong>` : ''}.
    </p>
  ...
  <div class="dna-edge">${WB.dna(13, 14)}</div>
</div>`;

    // use:enhance → busy state: flip button labels/disabled, then let the native POST go.
    const form = el.querySelector('form.card');
    ...
    form.addEventListener('submit', () => {
      ...
      approve.textContent = 'Authorizing…';
    });
  }
});
