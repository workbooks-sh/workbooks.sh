...
Every visual value comes from a token in [[file:../theme-contract.json][theme-contract.json]] — the single
...
=applyTheme(spec.theme, {mode})= ([[file:../src/theme/registry.js][registry.js]]) which writes the =--work-*=
