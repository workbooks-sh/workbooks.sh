...
export default defineConfig({
  ...
  server: {
    ...
    /* Extra dirs added to Vite's serving allow-list. Used in git-worktree
     * dev where node_modules is symlinked to the main checkout (outside the
     * worktree root), which Vite otherwise 403s. Colon-separated absolute
     * paths via WB_FS_ALLOW (or legacy WB_VITE_FS_ALLOW). No effect when
     * unset (normal builds). */
    fs: (process.env.WB_FS_ALLOW || process.env.WB_VITE_FS_ALLOW)
      ? {
          allow: [
            ".",
            ...(process.env.WB_FS_ALLOW || process.env.WB_VITE_FS_ALLOW || "").split(":"),
          ],
        }
      : undefined,
  },
