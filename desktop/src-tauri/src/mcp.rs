...
fn tool_manifest() -> Value {
    json!([
        ...
        tool("open_workbook", "Open a .html/.org workbook by path (focuses if already open).",
            json!({ "path": { "type": "string" } })),
        tool("weave", "Render (weave) a workbook — a workbook IS HTML, so this is passthrough.",
            json!({ "org": { "type": "string" } })),
        tool("validate", "Validate a workbook's work-* component graph.",
            json!({ "org": { "type": "string" } })),
        tool("outline", "Outline a workbook — the work-* element structure.",
            json!({ "org": { "type": "string" } })),
