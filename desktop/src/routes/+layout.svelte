<script lang="ts">
  ...
  import { chrome as chromeUi } from "$lib/ui/chrome.svelte";

  // Demo-recorder hooks — EXPOSED ONLY in the browser-preview mock, so the
  // product-showcase recorder can hot-swap the sidebar layout, open the share
  // flow, and close drawers from eval_js (no settings detour). No-op in the app.
  if (typeof window !== "undefined" && (window as unknown as { __WB_DEV_MOCK__?: boolean }).__WB_DEV_MOCK__) {
    Object.assign(window as unknown as Record<string, unknown>, { __wbNav: nav, __wbShare: createPackage, __wbChrome: chromeUi });
  }
