<script lang="ts">
  ...
  import { verifyHtml, type VerifyReport, type VerifyFail } from "../verify";
  ...
  let resolvedIssuer = $state<BrokerIdentity | null>(null);
      ...
      if (r.ok) {
        try {
          ...
          if (!(e instanceof DemoModeError)) {
            // Real failure (e.g. network) — just don't show a handle;
            // the badge still surfaces "verified" without naming the
            // person.
            resolvedIssuer = null;
          }
        }
  ...
  // Issuer info comes ONLY from the Broker. The "demo PEOPLE
  // fixture .verified → synth real-name" path was removed because
  // the audit flagged it as forging the "verified · real name"
  // badge tier for any handle in the local fixture.
  const issuerHandle = $derived<string | null>(resolvedIssuer?.handle ?? null);
