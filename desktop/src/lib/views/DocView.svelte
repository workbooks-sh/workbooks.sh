<script lang="ts">
   ...
   * focused. It loads the file by path (read_file), and for workbook/org kinds
   * weaves Org → HTML live via the embedded oql.wasm kernel — no runtime, no
   * Docker, offline. Edits set the tab dirty; ⌘S writes back (write_file) and
   * clears it. Code/text kinds show the source without a weave panel.
   *
   * This replaces the old "Document viewer lands here" placeholder — it makes
   * the tab system a real, usable local editor end-to-end (wb-jay.7).
   */
  ...
  const wovenKind = $derived(active?.kind === "org" || active?.kind === "workbook");
  ...
  function onInput() {
    ...
    if (!wovenKind || !hasLocalKernel()) return;
    clearTimeout(timer);
...
{#if active}
  <section class="doc">
    ...
    <div class="body" class:woven={wovenKind}>
      {#if wovenKind}
      ...
      <div class="main">
        <textarea bind:value={text} oninput={onInput} spellcheck="false"></textarea>
        {#if wovenKind && html}
...
{:else}
  <div class="empty">
    ...
    <p class="muted">Open a file from the rail, search, or bookmarks to start.</p>
  </div>
{/if}
...
<style>
  ...
  .hl {
    padding: 0.15rem 0.25rem; white-space: nowrap; overflow: hidden;
    text-overflow: ellipsis; border-radius: 4px;
  }
  .hl-state { color: #d19a66; font-style: normal; font-size: 0.72rem; margin-right: 0.35rem; }
  .muted { opacity: 0.5; }
  .main { display: flex; flex-direction: column; gap: 0.75rem; flex: 1; min-width: 0; min-height: 0; }
  textarea {
