...
const performOp = (scope, op, left, right, leftType, rightType) => {
  ...
  // strict (no-guess) known types — a wrong guess must not suppress the
  // runtime string-concat check for `+`
  const strictKnownLeft = knownType(scope, leftType);
  const strictKnownRight = knownType(scope, rightType);
  const isStrStrict = t => t === TYPES.string || t === TYPES.bytestring || t === TYPES.stringobject;
  const plusNeedsRuntimeStrCheck = !(
    strictKnownLeft != null && strictKnownRight != null &&
    !isStrStrict(strictKnownLeft) && !isStrStrict(strictKnownRight)
  );
  ...
  // When we emit a runtime string check, the operand TYPE expressions
  // (leftType/rightType) may be runtime `getLastType()` reads. Evaluating the
  // right operand clobbers #last_type, so a late re-read of leftType would see
  // the wrong type (e.g. number+stringCall -> both look like string -> strcat on
  // a number pointer -> OOB). Capture each operand's type into an i32 tmp right
  ...
  if (op === '+' && plusNeedsRuntimeStrCheck) {
    ...
    ops.unshift(
      ...
      ...eitherStringType(lType, rType),
      [ Opcodes.if, Blocktype.void ],
      ...concatStrings(scope, [ [ Opcodes.local_get, tmpLeft ] ], [ [ Opcodes.local_get, tmpRight ] ], lType, rType),
      [ Opcodes.br, 1 ],
      [ Opcodes.end ],
  ...
  return finalize([
    ...
    // capture left's type before evaluating right (which clobbers #last_type)
    ...(useTypeTmps ? [ ...leftType, [ Opcodes.local_set, tmpLeftType ] ] : []),
    ...right,
    ...(tmpRight != null ? [ [ Opcodes.local_tee, tmpRight ] ] : []),
    ...(useTypeTmps ? [ ...rightType, [ Opcodes.local_set, tmpRightType ] ] : []),
    ...ops
  ]);
};
...
const getNodeType = (scope, node) => {
  ...
  const ret = (() => {
    ...
    if (node.type === 'BinaryExpression') {
      ...
      // strict (no-guess) known types: only conclude the result is statically a
      // number when BOTH sides are strictly known non-strings. If a side's type
      // is only *guessed* (e.g. an unknown-typed call that could return a string
      // at runtime), defer to the runtime last-type — else console.log etc.
      // would format a runtime string pointer as a number.
      const sKnownLeft = knownType(scope, leftType);
      const sKnownRight = knownType(scope, rightType);
      const sIsStr = t => t === TYPES.string || t === TYPES.bytestring || t === TYPES.stringobject;
      if (
        sKnownLeft != null && sKnownRight != null &&
        !sIsStr(sKnownLeft) && !sIsStr(sKnownRight)
      ) return TYPES.number;
