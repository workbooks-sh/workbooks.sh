...
i32.store8 0 12`;
  ...
  if (Porffor.wasm`local.get ${separator+1}` == Porffor.TYPES.regexp) {
    ...
    while (lastIndex < thisLen && outLen < limit) {
      ...
      // ECMAScript 22.1.3.21: insert each of the match's capture groups after the preceding substring.
      const ncap: i32 = match.length; // = 1 (whole match) + capture count
      for (let ci: i32 = 1; ci < ncap; ci++) {
        if (outLen >= limit) break;
        outLen = Porffor.array.fastPush(out, __Array_prototype_at(match, ci));
      }
