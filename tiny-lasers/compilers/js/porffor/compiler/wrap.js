...
export default (source, module = undefined, print = str => process.stdout.write(str)) => {
  ...
  // Host-call bridge (wb host-bridge prototype). Single raw memory-exchange import: the guest names an
  // operation via a byte-region (opPtr,opLen), hands the host a request payload region (reqPtr,reqLen)
  // and a caller-allocated result region (resPtr,resCap); the host reads op+request, writes up to resCap
  // result bytes into resPtr, and returns the result length (or -1 on overflow / unknown op). All params
  ...
  createImport('ho_delete', [ i32, i32 ], [ i32 ], () => 0);
  // Override the positional single-char idents with meaningful field names (named-import path).
  for (const n of [ 'ho_new', 'ho_set', 'ho_get_value', 'ho_get_type', 'ho_has', 'ho_delete' ]) {
