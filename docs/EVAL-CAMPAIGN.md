...
- [~] **0.1a** HYGIENE (non-gaming — correct test tagging, not weakening). IN PROGRESS:
  ...
  - DONE: fixed a guardrail FALSE-POSITIVE — `cli_name_test.exs` now skips `#+GENERATED:` artifacts (examples/groundwork/BOARD.org is regenerated by Groundskeeper.Board from bd issue TITLES; it echoed historical "rename CLI: wb → wbx" text — a record, not a source ref). Guardrail back to 1/0.
  - REMAINING: tag the still-failing `clang_not_built`/build tests as `:build` — `command_registry_test` (the untagged "upper"/compile cases), `build_recipes_test`, `run_command_telemetry_test`, `brokered_app_test`. Goal: default suite 0-failure here. (~25 failures left, all BLOCKED:compiler-toolchain.)
