...
- **ToolkitInjection FIXED (real bug, Phase 1)**: the direct-verb toolkit guidance told the agent to run a DEAD command — `` `wb deploy status` `` (toolkits.ex:1230 hardcoded `"wb #{task}"`). The CLI is `work`; `wb` doesn't exist → the agent would fail. Fixed the suggestion to `work`, made the gate robust (`cli_bin in ["work","wb"]`), and renamed the `CLI_BIN: wb` sentinel → `work` in 5 manifests (workbooks-cli/browser/system, byod, publish). The guardrail missed this (bare `wb` it can't safely scan) → added a `CLI_BIN:\s*wbx?\b` pattern so it can't recur. toolkit_injection 11/0, guardrail 1/0, compile clean. Commit: <pending>.
