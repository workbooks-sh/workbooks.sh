...
- [x] **0.2** DONE — all keyless/keyed baseline gates recorded GREEN: guardrail 1/0, **design-lint PASS (78 element decls / 3 themes, 0 violations)**, **scaffold eval 4/4** (`node --test tools/scaffold.test.mjs`), agent_evals 9/9 (minimax-m3), agent_capabilities 4/4. Component-emit (Chrome+key+dist) deferred to its own item (dist bundle unbuilt). Note: claim-catalog says "45 work-* elements" but design-lint now counts 78 decls — the surface grew; not a failure.
...
- Loop robustness (dead-stop/runaway/unknown-tool/parallel; the empty-turn nudge fix) · `mix test test/agent_loop_test.exs` · GREEN(verified 31/0 cluster)
- Streaming carries tool_calls (multi-step chains; wb-0lw8 root fix) · `mix test test/agent_socket_bridge_test.exs` + capability eval · GREEN(verified)
- System-prompt composition · `mix test test/agent_prompt_test.exs` · GREEN(verified)
- vfs_write/read workdir confinement · `mix test test/vfs_write_confinement_test.exs` · GREEN(verified)
...
- Component-emit eval (right tag, data-bound, theme-honest, voice-parity) · `Workbooks.Evals.Components.run()` · RUNNABLE+1/1 — was NOT key/Chrome-blocked (both present); the real blocker was the **dist bundle couldn't build** (`workponents/package.json` declared zero deps but `tools/build.js` imports esbuild + the elements import lit). FIXED: declared `lit`+`esbuild` → bundle builds (525KB). `run("chart-not-table")` = 1/1 (tag=work-chart, emits/binds/themes/voice/rubric all ✓). Full 11-spec pack is the follow-up (slow, vision-judged).
...
- **Component-emit eval UNBLOCKED (real build-dep fix) + 1/1**: it was wrongly marked BLOCKED:Chrome+key — both are present (Chrome + the OpenRouter key that also routes the gemini judge). The ACTUAL blocker: `workponents/dist/workponents.js` couldn't build because `package.json` declared ZERO deps while `tools/build.js` imports `esbuild` and the elements import `lit` (the Lit re-base). A clean checkout literally cannot build the component bundle. FIXED: declared `lit ^3.2.0` + `esbuild ^0.24.0` → `npm run build` produces the 525KB bundle. Then `mix run -e 'Workbooks.Evals.Components.run("chart-not-table")'` (real minimax-m3 emit + gemini-via-OpenRouter vision judge + freshly-built dist mounted in headless Chrome) = **1/1**: agent emitted `work-chart` (not a table), emits_tag✓ binds_data✓ themes_from_tokens✓ voice_parity✓ rubric.passes✓. Compile clean, guardrail 1/0. Full 11-spec pack = follow-up. Commit: <pending>.
...
- **Sidebar DATA LAYER validated + desktop Rust backend 32/0** (wb-9s6v progress): refactored `package_workbooks` → pure `scan_workbooks(folders)` + 2 real isolated tests (spec-marked .html IS listed → agent/bundle workbook appears; marker-less/.txt NOT; empty/missing folder → [] no panic). Full `cargo test --lib` = 32/0 — the native layer behind sidebar/file/tab. NOTE: only .html/.htm with the workbook-spec marker list (the one-file format); a bare .org would NOT appear (legacy/deprecating). FULL GUI e2e (built Tauri app asserts file VISUALLY in sidebar + opens) still owed → wb-9s6v. Guardrail 1/0. Commit: <pending>.
- **Production UI validation (user redirect) — sidebar file-sync GAP found + FIXED**: traced the real loop (agent vfs_write → OS workdir ✓; `work app open-tab` → desktop:control push → tab opens + reads content ✓; sidebar lists via Tauri package scan ✓). GAP: the sidebar auto-refreshed ONLY on the native `fs-tree-changed` event and IGNORED the runtime `monorepo:watch` push — so a runtime-written agent file did not sync when the OS watcher missed it. FIX: `package.svelte.ts` now also subscribes `ws.onMonorepoChange` → coalesced `refresh()`. svelte-check clean (errors are pre-existing workponents host.js `any`s), guardrail 1/0. HONEST: full e2e (agent file VISIBLE in sidebar + opens) needs the BUILT TAURI APP — the vite harness MOCKS `package_list`, so it cannot prove the real sidebar. Filed **wb-9s6v** (Tauri-app production e2e + the cloud-nexus sidebar-sync architectural gap). Commit: <pending>.
- **Phase 3 — render-parser adversarial test ADDED 10/10**: new `desktop/src/lib/chat/messageRender.test.ts` (vitest) hardens `splitOrg`/`renderMode` (the UNTRUSTED-agent-output → component-segment seam). Proves: unknown :type preserved (renderer falls back), missing :type → callout default, malformed block → prose not a throw, late marker (>64 chars) → plain, INJECTION (</script>/onerror/<script> in body+props) captured as DATA verbatim (not stripped/executed — sink neutralizes), ReDoS-shaped 5000-block input finishes <2s, interleaved order preserved. Real can-fail assertions. Guardrail 1/0. Commit: <pending>.
- **Desktop MOCK e2e VERIFIED 11/11** (Phase 1/4): `app`+`features`+`adversarial` specs (vite mock providers, no runtime) — app boots offline + no sign-in gate, nexus popover Personal/local, workspace ⋯ Edit/Leave, onboarding gate, New folder modal, chat-as-tab, + adversarial negatives (share hidden on local nexus, nexus selection never re-prompts sign-in, Cancel dismisses). The desktop UI surface holds. Guardrail 1/0. Commit: <pending>.
- **Phase 4 — live desktop-UI e2e VERIFIED 2/2**: stood up the full stack (runtime `WB_WEB=1 WB_LLM_MODEL=minimax/minimax-m3` + vite `VITE_WB_RUNTIME_URL` + chromium; cleared the secrets file so the runtime uses the real env key) and ran `agent-live.spec.ts` (`WB_LIVE_E2E=1`): the REAL agent through the desktop UI (1) renders an inline component VISUALLY (painted + data-bound + themed, not just mounted), (2) creates a workbook (acts + confirms in the transcript). The "Waldo works end-to-end IN THE APP" proof — fresh, live. Guardrail 1/0. Commit: <pending>.
