...
- [ ] **0.1a** HYGIENE (non-gaming — correct test tagging, not weakening): tag the compiler-toolchain-dependent tests (the `clang_not_built` ones: wavelet_command, command_registry "upper"/compile cases, build_recipes, …) with `:build` (or a new `:compilers` tag) so the DEFAULT `mix test` excludes them like the existing `:build` tag — they still run on provisioned machines / CI with the toolchain. Goal: the default suite is 0-failure here, so a NEW failure stands out. This is the prerequisite for the loop to detect real regressions.
