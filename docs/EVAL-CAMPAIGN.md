...
- [x] **0.1a** DONE — default `mix test --seed 0` = 1071 tests, **2 failures, 0 `clang_not_built`** (was 49, all toolchain). The 2 remaining are NOT toolchain → Phase 1: `TenantIsolationHttpTest` OQL `/tmp/.org` confinement (known bead) + `ToolkitInjectionTest` direct-verb (RuntimeError — triage). Beyond those, a timing-flaky tail (same seed → different failures run-to-run): **wb-94qn**. Detail below:
...
- **0.1a DONE**: `@moduletag :build` on `dock_component_e2e_test.exs` (last clang straggler). `mix test --seed 0` = 1071 tests, **2 failures, 0 clang_not_built** (49→2 over the campaign). Guardrail 1/0, compile clean. The 2 stable remainders are real, NOT toolchain: TenantIsolation OQL `/tmp/.org` confinement + ToolkitInjection RuntimeError. ALSO confirmed wb-94qn is TIMING flakiness not pure order: same seed-0 gives different failures run-to-run (DesktopControl/FfmpegBroker pass alone, flake under suite load via assert_receive timeouts). Commit: <pending>.
