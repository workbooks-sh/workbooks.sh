...
- [~] **0.1a** HYGIENE (non-gaming — correct test tagging, not weakening). IN PROGRESS:
  ...
  - DONE: tagged `command_registry_test`'s 11 `upper` (source-built, needs clang) tests `:build` — they were the untagged compiled-command cases (file had 4 tagged already). File now 40 tests / 0 fail (15 excluded). Full suite 25→**13** failures.
  - REMAINING: `build_recipes_test`, `run_command_telemetry_test`, `brokered_app_test` (`clang_not_built`/build) + the `invalid` setup_all failures (`WasmShellTest`, `Workbooks.FabricTest`, …) — all BLOCKED:compiler-toolchain. Tag `:build`/moduletag next fire. Goal: default suite 0-failure here.
