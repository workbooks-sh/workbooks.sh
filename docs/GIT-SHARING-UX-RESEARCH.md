...
3. **Deployment** (`deploy.ex` + `deployment.org`) — `work deploy ci {github|gitlab|generic}` (`wb-6ttc.1` DONE) scaffolds CI that runs `work deploy validate` on PRs and `apply`+`verify` on main, **from `deployment.org` as source of truth**. `TENANCY_MODE`/`AUTH`/`PROFILE` flow to the engine as env. This is the git-workflow-native CI/CD layer.
