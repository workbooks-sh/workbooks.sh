defmodule Nexus.WashyVfsTest do
  @moduledoc """
  Washy's virtual filesystem is **pluggable** and **tenant-safe**. The `:map` backend is per-run
  process state; the `{:store, tenant}` backend persists into the tenant-partitioned SQLite store.
  These prove (1) the seam round-trips, (2) the SAME guest path in two tenants resolves to two
  disjoint byte-stores — there is no filesystem to escape, only a key in your own partition.
  """
  use ExUnit.Case, async: false

  alias Nexus.Washy.{VFS, FileRow}

  setup do
    Nexus.Store.clear(FileRow, "tenant-a")
    Nexus.Store.clear(FileRow, "tenant-b")
  ...
  test "{:store, tenant} backend persists into the tenant-partitioned SQLite store" do
    ...
    assert Nexus.Store.count(FileRow, "tenant-a") == 1
    VFS.delete("notes.txt")
    assert VFS.get("notes.txt") == nil
    assert Nexus.Store.count(FileRow, "tenant-a") == 0
  end
