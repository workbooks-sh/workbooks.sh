...
combined = hp <> "\nhostCall(\"echo\", \"\");\n" <> driver
{:ok, wasm} = Nexus.Compilers.Js.Porffor.compile(combined, Nexus.Compilers.Shared.default_root(), flags: ["--pageSize=65536", "--undefResilient"])
{:ok, mod} = TinyLasers.Wasm.decode(wasm)
Process.put(:tl_oob_debug, true)
...
try do
  ...
  :throw, {:wasm_exc,_,[ptr,t]} ->
    ...
    IO.puts("THROW t=#{t} MSG=[#{TinyLasers.Wasm.read_bytes(mem,mp+4,min(max(len,0),120))}] OUT=[#{dump.()}]")
end
