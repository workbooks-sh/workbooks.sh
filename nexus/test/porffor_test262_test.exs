defmodule Nexus.PorfforTest262Test do
  @moduledoc """
  **test262 conformance gate** — the official ECMAScript spec tests, run on the Porffor→Washy WASM→BEAM
  ...
  `Nexus.Porffor.Debug.diagnose(transpile: true)`, classify against the spec expectation.
  ...
  BEAM** (exactly the ceiling `Nexus.Porffor.Debug`'s docs + `run_corpus.exs` warn about). A SIGABRT cannot
