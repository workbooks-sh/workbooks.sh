defmodule Nexus.WashyAsmHostcallTest do
  @moduledoc """
  **Proof that WASI/host-import calls are ASM-NATIVE, not an interp fallback (the §0-B I/O bar).**
  ...
  lowered by `TranspileAsm` to a `call_ext` into `Nexus.Washy.invoke_host/2` — a native BEAM call across
  ...
  test "a guest function whose body is a WASI host-import call lowers to ASM (no interp fallback)" do
    ...
    interp = Washy.call_io(m, "f", [], transpile: false) |> elem(0)
    asm = apply(am, af, [])
