defmodule Nexus.WashyAsmTablesTest do
  @moduledoc """
  **Table-driven ops on the BEAM-assembly lane (`Nexus.Washy.AsmOps.Tables`).** `br_table` (multi-way
  ...
  **asm-native == forms-native == interpreter, bit-identical.** We A/B `call_io(transpile: true,
  ...
  test "br_table dispatches incl. out-of-range→default, asm == interp == forms" do
    ...
    for sel <- [0, 1, 2, 3, 7, 0xFFFFFFFF] do
      {interp, _} = Washy.call_io(m, "f", [sel], transpile: false)
      {forms, _} = Washy.call_io(m, "f", [sel], transpile: true, tier_threshold: 1, tier_async: false)
  ...
  test "call_indirect hits 2+ table entries, asm == interp == forms" do
    ...
    for {slot, x} <- [{0, 5}, {0, 0}, {1, 5}, {1, 7}, {0, 0xFFFF}, {1, 0xFFFF}] do
      {interp, _} = Washy.call_io(m, "f", [slot, x], transpile: false)
      {forms, _} = Washy.call_io(m, "f", [slot, x], transpile: true, tier_threshold: 1, tier_async: false)
  ...
  test "call_indirect bad index traps :undefined_element in both lanes" do
    ...
    assert_raise Nexus.Washy.Trap, fn -> Washy.call_io(m, "f", [5, 1], transpile: false) end

    assert_raise Nexus.Washy.Trap, fn ->
