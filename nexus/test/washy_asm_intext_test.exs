defmodule Nexus.WashyAsmIntExtTest do
  @moduledoc """
  **IntExt op-group (`Nexus.Washy.AsmOps.IntExt`) for the BEAM-assembly lane.** A/Bs globals, select,
  ...
  defp traps?(m, args) do
    fn lane_opts ->
      try do
        Washy.call_io(m, "f", args, lane_opts)
        false
      rescue
        Nexus.Washy.Trap -> true
      end
    end
  end
  ...
  for {name, op} <- [{"div_s", 0x6D}, {"div_u", 0x6E}, {"rem_s", 0x6F}, {"rem_u", 0x70}] do
    test "i32 #{name} divide-by-zero traps on BOTH lanes" do
      ...
      # asm lane directly
      {:ok, {am, af, _}} = TranspileAsm.try_emit(m, 0)
      assert_raise Nexus.Washy.Trap, fn -> apply(am, af, [5, 0]) end
    end
  end
  ...
  test "i32 div_s INT32_MIN / -1 overflow traps on BOTH lanes; rem_s does NOT" do
    ...
    assert_raise Nexus.Washy.Trap, fn -> apply(am, af, args) end
  ...
  defp agree_globals!(name, m, argsets) do
    ...
    for args <- argsets do
      {interp, _} = Washy.call_io(m, "f", args, transpile: false)
      {forms, _} = Washy.call_io(m, "f", args, transpile: true, tier_threshold: 1, tier_async: false)
      prev = Process.put(:washy_globals, Washy.init_globals(m))
      asm = apply(am, af, args)
