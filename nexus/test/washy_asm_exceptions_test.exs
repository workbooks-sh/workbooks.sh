defmodule Nexus.WashyAsmExceptionsTest do
  @moduledoc """
  ...
  test "throw: asm-emitted (no fallback) AND propagated {:wasm_exc} == interp" do
    ...
    interp = outcome(fn -> Washy.call_io(m, "f", [], transpile: false) end)
  ...
  test "throw_ref: re-raises an exnref's exception, and traps on null/non-exnref — asm == interp" do
    ...
    for {input, expected} <- [
      ...
      interp = outcome(fn -> Washy.call_io(m, "f", [input], transpile: false) end)
