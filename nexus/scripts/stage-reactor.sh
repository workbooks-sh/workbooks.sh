...
#   nexus/scripts/stage-reactor.sh
set -eu
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
command -v zig >/dev/null 2>&1 || { echo "[stage-reactor] zig not found (need 0.16) — skipping" >&2; exit 0; }
cd "$ROOT/reactor"
zig build reactor
mkdir -p "$ROOT/nexus/priv"
cp -f zig-out/bin/work-toolchain.wasm "$ROOT/nexus/priv/work-toolchain.wasm"
echo "[stage-reactor] staged work-toolchain.wasm → nexus/priv ($(du -h "$ROOT/nexus/priv/work-toolchain.wasm" | cut -f1))"
