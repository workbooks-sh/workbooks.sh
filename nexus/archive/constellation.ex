defmodule Nexus.Constellation do
  @moduledoc """
  Constellation — local two-brain inference, scheduled from nexus.

  **Experimental, OFF by default.** OpenRouter stays the primary LLM path; Constellation is an opt-in lane
  that routes a turn to a *local* model server instead. Enable per app env:

      config :nexus, Nexus.Constellation,
        enabled: true,
  ...
  Two lanes by **resource shape** (see `Nexus.Constellation.Lane`): a parallel CPU lane (autoregressive,
  ...
  """
  alias Nexus.{Llm, Constellation.Lane}

  @cpu Nexus.Constellation.CPU
  @gpu Nexus.Constellation.GPU

  @doc "True when the local lanes are enabled (else callers should fall back to OpenRouter)."
  def enabled?, do: cfg(:enabled, false)

  @doc "Recommended lane config for this machine (cores/memory/GPU). See `Nexus.Constellation.Tier`."
  def tier, do: Nexus.Constellation.Tier.recommend()
  ...
  @doc "Lane for an untagged free-text prompt. See `Nexus.Constellation.Router`."
  defdelegate classify(text), to: Nexus.Constellation.Router

  @doc """
  Run one local completion turn for `type`. The base lane comes from `route/1`, then
  `Nexus.Constellation.Router.dispatch/2` may spill a backed-up GPU job to an idle CPU lane.
  Returns `Nexus.Llm.complete/2` shape.
  """
  def run(type, messages, opts \\ []) do
    lane = Nexus.Constellation.Router.dispatch(type, route(type))
    GenServer.whereis(lane_name(lane)) || raise "Nexus.Constellation not started (enabled: true?)"
  ...
  def children do
    [
      {Task.Supervisor, name: Nexus.Constellation.Tasks},
