defmodule Nexus.Extract.Elixir do
  @moduledoc """
  ...
  Reads `Nexus.Literate` `:code` nodes that carried an `:ast` (the Elixir blocks).
  Returns one shape — `%{exports, types, calls}` — the SAME shape every per-language
  extractor returns, so `Nexus.Graph` (§9) and the WIT generator (§2) merge them
