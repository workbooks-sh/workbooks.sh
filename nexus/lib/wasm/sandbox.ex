defmodule Nexus.Wasm.Sandbox do
  @moduledoc """
  The **bounded run harness** for untrusted wasm — the one entry point production uses to execute a
  guest. It wraps `TinyLasers.Wasm.call_io/4` in a fresh, isolated, time-bounded process so a hostile or
  ...
  @ctx_keys [:tl_vfs, :tl_fds, :tl_nextfd, :tl_argv, :tl_stdin, :tl_backend, :tl_clock, :tl_out,
  ...
  def run(%Washy{} = mod, name, args, opts \\ []) when is_list(args) do
    ...
    case if(Keyword.get(opts, :validate, true), do: TinyLasers.Wasm.Validate.validate(mod), else: :ok) do
  ...
  def run_command(wasm, stdin, opts) when is_binary(wasm) do
    case TinyLasers.Wasm.decode_cached(bytes(wasm)) do
      {:ok, mod} ->
  ...
  defp exec_porffor(mod, opts) do
    ...
    if transpile?, do: TinyLasers.Wasm.Transpile.prewarm_bounded(mod, "m", true)
  ...
  def run_command({:interp, interp, source}, stdin, opts) do
    case TinyLasers.Wasm.decode_cached(bytes(interp)) do
      {:ok, mod} ->
  ...
  defp exec_module(mod, argv, stdin, vfs, opts) do
    ...
    if transpile?, do: TinyLasers.Wasm.Transpile.prewarm_bounded(mod, "_start")
