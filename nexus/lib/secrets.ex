defmodule Nexus.Secrets do
  ...
  # The org that owns this nexus — the CANONICAL `Nexus.Auth.nexus_org/0` (NEXUS_TENANT pin → founding
  # owner's org → default). Must match the org the dashboard WRITES secrets under, or a value set in the
  # UI never reaches the runtime; and the SAME definition the git-ingress gate uses (they must agree).
  defp secrets_org, do: Nexus.Auth.nexus_org()
  ...
  @privileged_static ~w(
    ...
    TELNYX_API_KEY FISH_API_KEY
    CLOUDFLARE_API_TOKEN CF_AIG_TOKEN EMAIL_API_KEY AGENTMAIL_API_KEY
