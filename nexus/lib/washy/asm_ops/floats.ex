defmodule Nexus.Washy.AsmOps.Floats do
  @moduledoc """
  **Float op-group for the BEAM-assembly lane (`Nexus.Washy.TranspileAsm`).** Lowers wasm f32/f64 ops to
  BEAM assembly, **bit-identical to the interpreter** by construction: every op emits a `call_ext` to the
  SAME `Nexus.Washy.Transpile`/`:erlang`/`:math`/`Float` helper the abstract-forms lane uses (or a `gc_bif`
  ...
  Frame model is the shared `Nexus.Washy.AsmCtx` contract: operands live in `y`-registers; `x0`/`x1` are
