defmodule Nexus.Washy.AsmOps.Exceptions do
  @moduledoc """
  **Exception ops (exnref, WASIX §0) for the BEAM-assembly lane.** Lowers `throw` and `throw_ref` to a
  `call_ext` of `Nexus.Washy.guest_throw/2` / `guest_throw_ref/1` — the SAME `{:wasm_exc, tag, vals}`
  ...
  defp do_throw(tagidx, s) do
    arity = Nexus.Washy.tag_arity_of(s.mod, tagidx)
