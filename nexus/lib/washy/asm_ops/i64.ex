defmodule Nexus.Washy.AsmOps.I64 do
  @moduledoc """
  **i64 op-group for the BEAM-assembly lane (`Nexus.Washy.TranspileAsm`).** Lowers wasm 64-bit-integer
  ops to BEAM assembly, bit-identical to the interpreter (`Nexus.Washy`) and the abstract-forms lane
  (`Nexus.Washy.Transpile`). i64 values are stored UNSIGNED in `0..2^64-1`; arithmetic wraps mod 2^64
  ...
  Frame model per `Nexus.Washy.AsmCtx`: persistent values in y-registers, `x0`/`x1` scratch, operand at
  ...
  (`div_s/2`, `div_u/2`, `rem_s/2`, `rem_u/2`, `rotl/2`, `rotr/2`), which reuse `Nexus.Washy.Trap` for
  ...
  @doc false
  def div_u(_a, 0), do: Nexus.Washy.Trap.trap!(:div_by_zero)
  def div_u(a, b), do: div(a, b) &&& @mask64
  @doc false
  def rem_s(a, b), do: irem(s64(a), s64(b)) &&& @mask64
  @doc false
  def rem_u(_a, 0), do: Nexus.Washy.Trap.trap!(:div_by_zero)
  ...
  defp idiv(_a, 0), do: Nexus.Washy.Trap.trap!(:div_by_zero)
  defp idiv(a, -1) when a == -0x8000000000000000, do: Nexus.Washy.Trap.trap!(:int_overflow)
  defp idiv(a, b), do: div(a, b)

  defp irem(_a, 0), do: Nexus.Washy.Trap.trap!(:div_by_zero)
