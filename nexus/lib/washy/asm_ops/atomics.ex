defmodule Nexus.Washy.AsmOps.Atomics do
  @moduledoc """
  **BEAM-assembly lowering of the wasm ATOMICS ops (WASIX §0).** An `AsmOps.*` op-group handler for
  `Nexus.Washy.TranspileAsm`. Like the Memory handler, each op lowers to a single `call_ext` into a
  `Nexus.Washy` host helper that holds the EXACT interpreter byte-math (`guest_load`/`guest_store` for
