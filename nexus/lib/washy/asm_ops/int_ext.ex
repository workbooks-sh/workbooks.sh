defmodule Nexus.Washy.AsmOps.IntExt do
  @moduledoc """
  **Extended-integer op-group for the BEAM-assembly lane (`Nexus.Washy.TranspileAsm`).** Lowers the i32
  ops the core i32 path doesn't cover — globals, `select`, shifts/rotates, div/rem, and bitcount — to BEAM
  assembly, **bit-identical to the interpreter (`Nexus.Washy`) and the abstract-forms lane
  (`Nexus.Washy.Transpile`)** by construction. i32 values are stored UNSIGNED in `0..2^32-1`.
  ...
  below (`div_s/2`, …, `rotl/2`, `rotr/2`), which reuse `Nexus.Washy.Trap` for div-by-zero / signed
  ...
  defp idiv(a, -1) when a == -0x80000000, do: Nexus.Washy.Trap.trap!(:int_overflow)
