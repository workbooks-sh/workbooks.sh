defmodule Nexus.Washy.HostProc do
  @moduledoc """
  ...
  `Nexus.Washy.host_exec/3` (the cooperative fork+exec+wait already proven against real Rust
  ...
  import Nexus.Washy, only: [read_bytes: 3, write_bytes: 3, host_exec: 2]
  ...
  defp fd_drain(fd) do
    case FdTable.get(fd) do
      %{kind: :pipe, ref: {pid, _end}} ->
        n = Nexus.Washy.FdTable.Pipe.available(pid)
        if n > 0, do: Nexus.Washy.FdTable.Pipe.read(pid, n), else: ""
