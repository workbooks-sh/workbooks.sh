defmodule Nexus.Washy.FdTable do
  @moduledoc """
  The ONE unified fd table for a Washy guest — the single source of truth for every fd kind
  ...
  closes (refcount → 0). Both Washy lanes (interpreter + BEAM-asm transpiler) route their WASI
  ...
  defp teardown(%{kind: :pipe, ref: {pid, role}}), do: Nexus.Washy.FdTable.Pipe.free(pid, role)
  ...
  """
  def pipe do
    pid = Nexus.Washy.FdTable.Pipe.new()
...
defmodule Nexus.Washy.FdTable.Pipe do
  @moduledoc """
