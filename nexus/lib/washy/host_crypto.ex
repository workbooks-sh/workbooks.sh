defmodule Nexus.Washy.HostCrypto do
  ...
  shape of the reference `Nexus.Washy.HostFs`. Backed by Erlang's `:crypto` (BoringSSL/OpenSSL), so there is
  no hand-rolled hashing. Binary data crosses the bridge base64-encoded (JSON-safe), exactly like host_fs.ex.

  Routed here by `Nexus.Washy.HostIO` because the import name is `crypto_*` (prefix `crypto` → this module).
