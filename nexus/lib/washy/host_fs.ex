defmodule Nexus.Washy.HostFs do
  ...
  file (no `harness_run.c`, no `washy.ex`). Reuses the existing `Nexus.Washy.VFS` (string-key → bytes,
  ...
  Routed here by `Nexus.Washy.HostIO` because the import name is `fs_*`. VFS reads/writes are synchronous
  ...
  def call("fs_read", [path]) do
    case Nexus.Washy.VFS.get(norm(path)) do
  ...
  def call("fs_write", [path, b64]) do
    Nexus.Washy.VFS.put(norm(path), Base.decode64!(b64))
    %{"ok" => true}
  end

  def call("fs_exists", [path]), do: %{"ok" => Nexus.Washy.VFS.has?(norm(path))}

  def call("fs_unlink", [path]) do
    Nexus.Washy.VFS.delete(norm(path))
    %{"ok" => true}
  end
