defmodule Nexus.Washy.SpecTest do
  @moduledoc """
  Runs WebAssembly **spec-test scripts** (`.wast`) against Washy: parse (`Sexp`) → assemble each
  ...
  import Bitwise
  alias Nexus.Washy
  alias Nexus.Washy.{Sexp, Wat, Validate}
  ...
  defp directive(["assert_return", ["invoke", {:string, fname} | args] | expected], state) do
    grade(state, fn mod ->
      actual = Washy.call(mod, fname, Enum.map(args, &eval_const/1))
  ...
  defp directive(["assert_trap", ["invoke", {:string, fname} | args] | _msg], state) do
    grade(state, fn mod ->
      try do
        v = Washy.call(mod, fname, Enum.map(args, &eval_const/1))
        {:fail, {:expected_trap, fname, v}}
      rescue
        Washy.Trap -> {:ok, :trapped}
      end
    end)
  end
