defmodule Nexus.Compilers.Js.Porffor.Census do
  @moduledoc """
  **Porffor conformance census — the data that drives the fork (Phase 1).**

  For each JS feature-probe: compile via Porffor + run on Washy → `got`; run the SAME source on host Node
  (the oracle) → `want`; classify. The node oracle is a **dev/test tool only** — never a runtime fallback;
  it exists to tell us *exactly which features Porffor miscompiles* so the Phase 2 fork is data-driven, not
  guesswork.

  `report/0` prints a ranked summary (per-category pass rate + every miscompile). `run/0` returns the raw
  results. Closures are the known linchpin (Porffor's `semantic()` pass, codegen.js:7578).

  Classification:
    * `:pass`        — Porffor output == node output (✅)
    * `:miscompile`  — both produced output, they DIFFER (🔴 the fork target)
    * `:crash`       — Porffor compile/run failed where node succeeded
    * `:oracle_skip` — node itself errored (bad probe — excluded)
  """
  alias Nexus.Compilers.Js.Porffor

  # {category, name, js}. Many fine-grained variants per category so a failure pinpoints the exact feature.
  @corpus [
    # ── arithmetic / control flow (expected: solid) ──
  ...
  @doc "The probe corpus (`{category, name, js}` triples)."
  def corpus, do: @corpus

  @doc "Run the whole census, returning `[{category, name, status, got, want}]`."
  def run do
  ...
  defp classify(js) do
    case node_oracle(js) do
      ...
      {:ok, want} ->
        case porffor_washy(js) do
          {:ok, got} -> {if(got == want, do: :pass, else: :miscompile), got, want}
          :error -> {:crash, nil, want}
        end
    end
  end
  ...
  @doc "Print a ranked human report (per-category pass rate + every miscompile/crash)."
  def report do
    results = run()
    ...
    IO.puts("\n══ PORFFOR CONFORMANCE CENSUS ══ (ranked worst-first)\n")
