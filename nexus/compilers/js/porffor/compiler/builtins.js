  ...
  _.__Porffor_malloc = {
    ...
    wasm: (scope, { builtin, glbl }) => [
      ...
      [ Opcodes.if, Valtype.i32 ],
        ...
        [ Opcodes.i32_add ],
        [ Opcodes.local_tee, 2 ],               // pages to grow
        [ Opcodes.memory_grow, 0 ],
        // memory.grow returns -1 when it would exceed the host's max pages — trap cleanly at the real OOM
        // point (the old code multiplied -1 by PageSize → a garbage base that trapped far away).
        [ Opcodes.local_tee, 1 ],               // old page count
        ...
        // capacity, a common off-by-one in object_hash/fastAdd) then lands in the unused guard page instead
        // of trapping at addr==memsize — which is exactly the OOB rollup hit during globalThis construction.
        [ Opcodes.local_get, 1 ],
        [ Opcodes.local_get, 2 ],
        [ Opcodes.i32_add ],
        number(PageSize, Valtype.i32),
