...
export const __Object_keys = (obj: any): any[] => {
  ...
  if (__Porffor_object_isProxy(obj)) return __Porffor_proxy_ownKeys(obj);
...
export const __Object_values = (obj: any): any[] => {
  if (obj == null) throw new TypeError('Argument is nullish, expected object');
  if (__Porffor_object_isProxy(obj)) {
    const keys: any[] = __Porffor_proxy_ownKeys(obj);
    const vout: any[] = Porffor.malloc();
    const klen: i32 = keys.length;
    vout.length = klen;
    for (let i: i32 = 0; i < klen; i++) vout[i] = __Porffor_proxy_get(obj, keys[i]);
    return vout;
  }
  const out: any[] = Porffor.malloc();
...
export const __Object_prototype_hasOwnProperty = (_this: any, prop: any) => {
  ...
  if (__Porffor_object_isProxy(_this)) return __Porffor_proxy_has(_this, p);
...
export const __Porffor_object_in = (obj: any, prop: any): boolean => {
  ...
  if (__Porffor_object_isProxy(obj)) return __Porffor_proxy_has(obj, prop);
...
local.set ${key}`;
        ...
        // A named (string/symbol) property set onto a TYPED ARRAY target must go to the array's underlying
        // object, NOT the element-store fast path: `ta[strKey] = v` in codegen treats the key as a numeric
        // index (key_ptr * elemSize + base → wild address) and corrupts memory. The read side already
        // resolves named props via __Porffor_object_underlying, so writing there round-trips correctly.
        const tt: i32 = Porffor.wasm`local.get ${target+1}`;
        const kt: i32 = Porffor.wasm`local.get ${key+1}`;
        if (tt >= 0x50 && tt <= 0x5a && (kt == Porffor.TYPES.bytestring || kt == Porffor.TYPES.string || kt == Porffor.TYPES.symbol)) {
          const u: any = __Porffor_object_underlying(target);
          u[key] = value;
        } else {
          target[key] = value;
        }
...
export const __Object_defineProperty = (target: any, prop: any, desc: any): any => {
  ...
  // A closure_convert BOX getter {__clo, env, fn} isn't a native function, so it can't be stored/called as a
  // plain accessor funcref — and a builtin can't call box.fn read off the object (traps). Instead, for a
  // getter-only boxed accessor (the esbuild/rollup `__copyProps` re-export shape: many `() => from[key]`
  // getters on one object), store the box's FN in the get slot and its ENV in the (unused) set slot, flagged
  // 0b10000; the read path calls `getFn(env)` from the slot — which works (loaded funcref), supporting any
  // number of boxed getters per object (the old closure_convert __defprop hack only allowed one).
  let getBoxed: boolean = false;
  ...
  if (get !== undefined || set !== undefined) {
    if (get !== undefined && Porffor.type(get) != Porffor.TYPES.function) {
      const gbox: object = get;
      if (Porffor.type(get) == Porffor.TYPES.object && gbox.__clo) getBoxed = true;
      else throw new TypeError('Getter must be a function');
    }
    if (set !== undefined && Porffor.type(set) != Porffor.TYPES.function) {
      const sbox: object = set;
      if (Porffor.type(set) == Porffor.TYPES.object && sbox.__clo)
        throw new TypeError('boxed setter unsupported');
      else throw new TypeError('Setter must be a function');
    }
    if (getBoxed && set !== undefined) throw new TypeError('boxed getter with setter unsupported');
  ...
  if (getBoxed) flags |= 0b10000;

  if (getBoxed) {
    // get slot = box.fn (a real funcref), set slot = box.env — read path calls getFn(env).
    const gbox: object = get;
    value = Porffor.object.packAccessor(gbox.fn, gbox.env);
  } else if (accessor) value = Porffor.object.packAccessor(get, set);
