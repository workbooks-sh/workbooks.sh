...
#+GENERATED: [2026-06-11 05:13 UTC] by Workbooks.Groundskeeper.Board — NEVER hand-edit (regenerated from bd; bd stays out of git)
...
** TODO deploy-kit P1: size fly machine for the runtime by default (>=1GB, rec 2GB) + #+DEPLOY_MEMORY knob + post-apply /health poll surfacing OOM — first cloud apply crash-loops on default 512MB. Found by lander-live.
   :ID: wb-c4w
** TODO SECURITY: deployed engine control plane open (x-tenant dev fallback) — wb deploy cloud should require a persisted bearer (WB_PUBLIC_BEARER) when not dev; /api/run + /rcp/* + PUT /w are spend/abuse surfaces. Found by lander-live eval.
   :ID: wb-oom
...
** TODO [epic] lander-live — self-maintaining lander as system proof: runtime-hosted page + hourly keeper agent + observable loop
   :ID: wb-5vm
** TODO [epic] CTK + eval SDK + wb dev — PRODUCT-complete (A) :epic:
   :ID: wb-3gm
...
* P2 (108)
** TODO groundskeeper: ledger orphan handling — persist tasks (or mark DOING→BLOCKED on boot) so a restart can't leave TASKS.org stuck doing
   :ID: wb-3ojf.7
...
** TODO groundskeeper v2: workbook-native mobile client
   :ID: wb-3ojf.5
** TODO groundskeeper v1: dispatch_research — voice-filed research lane a deep-research agent works async
   :ID: wb-3ojf.4
** TODO groundskeeper v1: BOARD.org generation — bd -> committed org Kanban, the git-visible single source of truth
   :ID: wb-3ojf.3
** TODO groundskeeper v0: ElevenLabs agent def — persona prompt + 3 server tools + post-call webhook
   :ID: wb-3ojf.2
** TODO groundskeeper v0: bridge endpoints on runtime web plane — repo_state, capture, file_issue (HMAC-authed)
   :ID: wb-3ojf.1
...
** DONE Agent shell tool: WASM shell wedges on the lander box (every call → 150s timeout) — health-check and omit, or fix provisioning
   :ID: wb-91j
** DONE Material Icon Theme as file/folder icon standard (emoji still overrides)
   :ID: wb-5fl.15
** DONE color system unification: tintFor everywhere, plain Create, bottom icon toolbar
   :ID: wb-5fl.14
** DONE sidebar polish: persistent dashed bookmarks container, folder drop-target move, folder ctx New workbook
   :ID: wb-5fl.13
** DONE bookmarks = explicit refs only: apps back to rows, grid only holds dragged-in pins, app-drag accepted
   :ID: wb-5fl.11
