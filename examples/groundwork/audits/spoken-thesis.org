#+TITLE: the spoken thesis, audited
#+DATE: <2026-06-10>
...
* Prong 3 — toolkits + org mode (the authoring surface)

** KEPT org mode is the authoring/config language: parsable, stateful, chronological, in every model's training data
   Quote: "what it does excel at for workbooks is being a configuration
   language."
   Evidence: lifecycle.org, agent defs, plan.org boards, toolkit manifests —
   the engine interprets org as the declarative layer. (His proposed
   benchmark — "ask models org mode from memory" — is a real, cheap eval to
   actually run.)

** KEPT toolkits = org skill-trees wrapping CLIs compiled to WASM, run via work toolkit
   Quote: "package up a CLI system by using a WebAssembly compiler… wrap them
   in the same skill structure."
   Evidence: six EXEC shapes audited; huniq autobuild proof; AUTHORING.org;
...
** CLAIMED org mode beats Markdown because drift is TESTABLE ("I can't write a test suite for Markdown")
   ...
   linter/validator for org artifacts (=work content check= exists — how much
...
* The argument structure (what the pitch actually is)
  ...
  4. Heterogeneous tooling makes agent data noise; a homogeneous authoring
     surface (org) makes drift testable, telemetry consistent, and —
...
* Open questions the recording leaves dangling
  ...
  - The org-mode model-knowledge benchmark: cheap to run, never run.
  - Drift detection: claimed as org's killer feature; how much is shipped?
