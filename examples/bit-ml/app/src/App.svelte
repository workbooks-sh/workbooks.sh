<script>
  // bit.ml — DESIGN COMPONENTS WORKBOOK.
  // The index IS the design system: a specimen sheet where every component
  // from DESIGN.md renders live, labelled like a type specimen. This page is
  // itself a brand artifact — paper background, the hairline rhythm. It must
  // FEEL like the product, not like a Storybook.
  ...
  // ── crew panel state (toggled from the masthead) ────────────────────────
  let crewOpen = $state(false);
  let crewFilter = $state(null);
  function openCrew(name = null) { crewFilter = name; crewOpen = true; }

  // ── specimen data (honest fakes; market numbers flagged · specimen) ─────
  const crew = [
    { name: 'wren', role: 'writer',     state: 'live', doing: 'drafting: deepmind weather' },
    { name: 'moss', role: 'research',   state: 'live', doing: 'pulling: nature paper' },
    { name: 'hale', role: 'editor',     state: 'idle', doing: 'idle — next pass in 4m' },
    { name: 'desk', role: 'assignment', state: 'live', doing: 'thinking' },
  ];
  const wire = [
    { time: '14:01', who: 'wren', msg: 'draft: deepmind-weather.html' },
    { time: '13:58', who: 'moss', msg: 'research note: 6 sources attached' },
    { time: '13:51', who: 'desk', msg: 'assigned: deepmind weather model' },
  ];
  const pipeline = 'assigned 2 · research 1 · writing 1 · edit 0';
  ...
  const tokens = [
    { name: '--paper', hex: '#faf9f6', note: 'warm newsprint white' },
    { name: '--ink',   hex: '#111213', note: 'near-black, warm' },
    { name: '--wire',  hex: '#0a52e0', note: 'the one accent' },
    { name: '--up',    hex: '#0a7d4f', note: 'market: up' },
    { name: '--down',  hex: '#c4322e', note: 'market: down' },
    { name: '--rule',  hex: '#e4e2dc', note: 'hairline' },
  ];
</script>
...
<div class="sheet">
  ...
  <!-- specimen intro line (mono) -->
  <p class="intro mono">
    bit.ml — design components. Every element below is rendered live from the
    real library. Captions name the component and the rule it embodies.
  </p>
  ...
  <section class="spec" id="type">
    ...
    <div class="type-opt">
      ...
      <div class="to-row"><span class="to-cap mono">Newsreader 22 / bite head</span><span class="serif n22">DeepMind's weather model beats the supercomputers</span></div>
    ...
    <p class="switzer-block">
      ...
      dek is informed, not teased: the text voice does the actual work of
      telling you what happened and what it costs.
    </p>

    <div class="meta-row mono">14:02 UTC · 40s read · sources: Nature · DeepMind blog</div>

    <!-- the rule that makes it sing (§2): serif and mono never touch without
         a hairline or whitespace between them. Shown explicitly, labelled. -->
  ...
  <section class="spec" id="stack">
    <div class="spec-label">
      ...
      <span class="spec-rule">rules and rhythm — no cards; states unread / read / live</span>
    </div>
    ...
    <Bite
      ...
      byline={{ writer: { name: 'desk', role: 'writer' }, research: ['moss'], editor: 'hale' }}
      onagent={openCrew}
    />
  ...
  <section class="spec" id="receipt">
    ...
    <Receipt
      title="deepmind-weather.html — the run"
      rows={[
  ...
  <!-- ── CREW PANEL (static open specimen, inline) ──────────────────── -->
  <section class="spec" id="crew">
    ...
    <CrewPanel inline agents={crew} {wire} {pipeline} clock="14:02:33" />
  </section>
...
<!-- the docked, toggleable crew panel (from the masthead) -->
<CrewPanel
  open={crewOpen}
  filter={crewFilter}
  agents={crew}
...
<style>
  ...
  .switzer-block {
    margin: 0 0 22px; font-size: 17px; line-height: 1.65; color: var(--ink-2);
    max-width: 60ch; letter-spacing: -0.01em;
  }
  .meta-row {
    font-size: 11px; text-transform: uppercase; letter-spacing: 0.06em;
    color: var(--ink-3); padding: 10px 0; border-top: 1px solid var(--rule);
  ...
  .td-gap {
    width: 1px; align-self: stretch; margin: 2px 14px;
    background: var(--rule);   /* the literal hairline between the voices */
  }
  ...
  /* — portal well: give the rim room to breathe on paper — */
