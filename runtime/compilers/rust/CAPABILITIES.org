#+TITLE: Rust-in-WASM — capabilities, limitations, and how to mitigate (wb-0sz)
#+NOTE: The honest contract for compiling Rust to wasm in-sandbox (mrustc.wasm → clang.wasm, no
#+NOTE: native rustc). Machine-readable twin: Workbooks.Compilers.RustCaps.{capabilities,diagnose}/_.
#+NOTE: If you are an agent or a person about to build something, read this FIRST — it tells you
#+NOTE: what the lane can't do and what to do instead, so you don't fight a wall.
