#+TITLE: Rust crates.io — in-sandbox coverage (wb-3s8)
#+NOTE: Real crates.io crates that compile + run ENTIRELY in the wasm sandbox as workbook deps
#+NOTE: (fetch from static.crates.io → mrustc.wasm → clang.wasm → link → run). Verified, not toys.

* PROVEN WORKING (16 crates as of 2026-06-07)
  | crate            | ver     | class                                      |
  |------------------+---------+--------------------------------------------|
