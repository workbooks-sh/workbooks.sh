defmodule Workbooks.KernelDispatchTest do
  @moduledoc """
  wb-pkh.11 — a toolkit declares #+EXEC: kernel and `wb toolkit build` produces a
  ...
  @tag :build
  test "wb toolkit build of an #+EXEC: kernel toolkit → registered kernel → open by name → run" do
