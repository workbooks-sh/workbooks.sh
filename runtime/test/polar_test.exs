defmodule Workbooks.Billing.PolarTest do
  use ExUnit.Case, async: true
  alias Workbooks.Billing.Polar
  ...
  describe "verify_webhook/3 — Standard Webhooks (the security keystone)" do
    # Canonical Standard-Webhooks / Svix test vector. The expected signature is a
    # FIXED, externally-derived value (verified against the spec's reference
    # libraries), so this also locks our signed-content + HMAC construction.
    ...
    # The static test vector uses a 2021 timestamp; we re-sign over a CURRENT
    # timestamp for the in-tolerance acceptance tests so the 5-minute replay
    # window doesn't reject a legitimately-signed message.
    ...
    test "accepts when ANY v1 entry in a space-separated list matches (key rotation)" do
      ...
      multi = "v1,AAAAbogusAAAA= " <> good <> " v1a,asymmetric-ignored"
    ...
    test "rejects a TAMPERED payload (body differs from what was signed)" do
      ...
      assert {:error, :no_matching_signature} =
               Polar.verify_webhook(~s({"test": 9999}), headers, @secret)
    end
    ...
    test "rejects an OLD timestamp (outside the 5-minute tolerance) → replay defense" do
      ...
      assert {:error, :timestamp_out_of_tolerance} =
               Polar.verify_webhook(payload, headers, @secret)
    end
    ...
    test "rejects a malformed (non-base64) secret" do
      {headers, payload} = valid_headers()
      assert {:error, :malformed_secret} =
               Polar.verify_webhook(payload, headers, "whsec_!!!not base64!!!")
    end
    ...
    test "uses a CONSTANT-TIME compare — no plain == on the signature" do
      src = File.read!(Path.join([__DIR__, "..", "host", "billing", "polar.ex"]))
      # crypto's constant-time equality must be present...
      assert src =~ ":crypto.hash_equals"
      # ...and there must be no `==` applied to the raw/expected signature binaries.
  ...
  describe "build_request/4 (pure construction, no network)" do
    test "ingest_usage targets POST /v1/events/ingest with the events envelope, NO auth header" do
      {method, url, headers, body} =
        Polar.build_request(:post, ["v1", "events", "ingest"],
          %{"events" => [%{"name" => "tok", "customer_id" => "c1"}]},
          token: @token
        )
    ...
    test "ingest_usage wraps a single event into a list" do
      ...
      assert {:ok, %{"inserted" => 1}} =
               Polar.ingest_usage(%{"name" => "tok", "customer_id" => "c1"},
                 http: http,
                 token: @token
               )
    end
  ...
  describe "token never leaks" do
    test "token absent from every returned value (ok, error, transport-error, invalid)" do
      ...
      results = [
        Polar.ingest_usage(%{"name" => "x", "customer_id" => "c"},
          http: fn _, _, _, _ -> {:ok, {200, "{}"}} end
        ),
  ...
  # CRITICAL regression lock: the Polar access token (org master credential) must
  # travel over VERIFIED TLS. With no :ssl options :httpc runs verify_none →
  # MITM-stealable. These assertions fail against that logic.
  describe "TLS is verified (no MITM of the Polar access token)" do
    test "http_options pin verify_peer, a non-empty CA store, and the api.polar.sh SNI" do
  ...
  describe "entitlements/1 (pure mapping; fail closed)" do
    test "active subscription → can_provision with tier limits" do
      sub = %{"status" => "active", "product" => %{"name" => "Pro plan"}}
      ent = Polar.entitlements(sub)
