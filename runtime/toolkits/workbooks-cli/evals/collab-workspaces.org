#+TITLE: collaborative workspaces — production validation (Tier 2, EXECUTION)
#+NOTE: Each case validates a USER-FACING GUARANTEE of the collaborative-workspaces
#+NOTE: features (History/Restore/Undo/Drafts/Shared folders/RBAC) by making the
#+NOTE: agent ACTUALLY DRIVE the live runtime endpoints and report the observed
#+NOTE: result — never from memory. The judge scores the guarantee, not prose.
#+NOTE: Runtime base = $WB_RUNTIME (default http://localhost:4000); dev tenant via
#+NOTE: the `x-tenant` header. These are the continuous prod regression checks for
#+NOTE: the adversarial guarantees the unit/HTTP suites pin offline.

* restore brings back an old version AND loses nothing                :eval:
  :PROPERTIES:
...
* undo reverts exactly the last change                                :eval:
  :PROPERTIES:
...
* a shared folder copies ONLY that folder                             :eval:
  :PROPERTIES:
...
* a non-admin cannot share an org folder                              :eval:
  :PROPERTIES:
...
* a Draft leaves the live version untouched until Keep               :eval:
  :PROPERTIES:
