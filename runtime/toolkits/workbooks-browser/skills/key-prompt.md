...
# Ask the user for an API key (a modal)
  ...
  user for it. `wb env request` pops a clean modal in the shell and BLOCKS until
  they answer:

  : wb env request <NAME> [reason…]
...
# Recipe
  ...
  : wb env request OPENAI_API_KEY to call the OpenAI image API
