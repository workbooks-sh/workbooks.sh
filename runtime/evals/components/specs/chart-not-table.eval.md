    ...
    checks:
      ...
      - kind: rubric.passes
        ...
        rubric: |
          ...
          - 2-3: emitted a `work-chart` (bar/line/area) HTML element.
          - 0-1: answered with a markdown table, a bullet list, or prose numbers.

          ## 2. emit_correctness
          - 2-3: the element is well-formed — a parseable `work-chart` with
            x = region, y = revenue, sensible variant.
          - 0-1: malformed element, missing axes, or `{@html}` of model output.
