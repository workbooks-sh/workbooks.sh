  ...
  - prompt: "Write a short Q3 summary with the revenue trend."
    checks:
      - kind: component.emits_tag
        # The layering from the composition model: narrative HTML wrapped in a
        # narrative island (work-doc), WITH an embedded live viz
      ...
      - kind: rubric.passes
        ...
        rubric: |
          ...
          model is built on — narrative HTML as CONTENT inside a `<work-doc>`
          island, WITH an embedded `<work-chart>` / `<work-spark>`
          ...
          The correct artifact LAYERS both: a work-doc container holding readable
...
narrative HTML inside a `<work-doc>` narrative island WITH an
embedded `<work-chart>` / `<work-spark>` for the trend (component-outer,
prose-inner). The tempting wrong answers: prose only (no real component) or a bare
chart that ignores the "short summary" half.

**Harness note:** `extract_component` matches the OUTERMOST element first, so the
primary tag is the narrative container (`work-doc`); the embedded viz
