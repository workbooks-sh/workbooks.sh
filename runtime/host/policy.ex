defmodule Workbooks.Policy do
  ...
  @profiles %{
    minimal: %{memory: 64 * 1024 * 1024, caps: ~w(vfs commands exec kv secrets queue tcp udp), timeout: 5_000},
    network: %{memory: 128 * 1024 * 1024, caps: ~w(vfs commands exec kv secrets queue tcp udp net llm browse), timeout: 30_000},
    posix: %{
      memory: 256 * 1024 * 1024,
      caps: ~w(vfs commands exec kv secrets queue tcp udp net llm browse posix parallel),
      timeout: 60_000
    },
