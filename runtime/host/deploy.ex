    ...
    case ci_spec(provider) do
      ...
      {path, body} ->
        if File.exists?(path) and not force? do
          ...
          ok(
            "wrote #{path} — commit it. CI now runs `work deploy validate` on PRs and `apply`+`verify` on main. Set the secrets your deployment.html references in your CI provider.",
