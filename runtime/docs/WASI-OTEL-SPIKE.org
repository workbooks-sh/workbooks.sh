#+TITLE: wasi-otel spike — WASM-path observability
#+STATUS: feasibility CONFIRMED; live guest wiring blocked on external transform tooling

* Question
  Telemetry phase 0b/0c/0e captured every NATIVE tool call (the BEAM
  =run=/=shell=/=fetch= tools) at the chokepoint in =Workbooks.Agent= →
