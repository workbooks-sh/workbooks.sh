#+TITLE: Toolkits V3 — Finish + Hardening Report
#+DATE: 2026-06-07
#+EPIC: wb-tk3
#+BRANCH: feat/toolkits-finish
#+STATUS: VERIFIED
#+RELATED: TOOLKITS-V3.org, ARCHITECTURE.org

* Scope

  Final verification of the toolkits-finish branch against the design of record
  (TOOLKITS-V3.org). This report records, for each declared gap, an honest
...
** P4 — Declarative EXEC / build descriptor — DONE
   ...
   =work toolkit build= drives the build per declared lang and refuses a CLI_BIN
   ...
   - "work toolkit build refuses a reserved CLI_BIN (no shadowing of jq)"
   - "work toolkit build huniq compiles the crate, registers it, and it runs"
...
** FAILING #2 — command_registry_test.exs:452 "very-long stdin (256 KiB) round-trips through upper"
  ...
  Recommend a one-line fix for each before shipping the =work toolkit show= TOC and
...
* 3. Security audit (wb-sec)
  ...
  schedule). Neither is a silent claim — both are documented in TOOLKITS-V3.org
