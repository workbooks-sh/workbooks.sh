#+TITLE: OQL Plugins — SaaS adapters as first-class plugins, no sidecar
#+DATE: 2026-06-02
...
federation pattern (base manifest + plugin/manifest.org + sync.org daemon) is
summarized in TOOLKITS-V3.org §"Federation".
#+END_COMMENT
...
* Reusing OQL's existing plugin system
  ...
  #+ENV_KEYS: GITHUB_TOKEN            # secrets DeployKit + work env must inject
...
* The auth shim
  ...
  | Context              | Source                                    |
  |---------------------+--------------------------------------------|
  | Local dev           | process env, then `work env` keychain        |
...
* DeployKit integration
  ...
  When a developer runs =work deploy= on their workbooks app:
...
* Read vs Write
  ...
  *Write* (mutations) is the v2 add-on — plugins that support it
  ...
  =bash= → =work plugin <id> <verb> <args>= CLI path).
...
* What we DON'T do
  ...
  - No `work` "plugin install" verb that downloads binaries.  Plugins
...
* Implementation order
  ...
  4. *Auth shim* (=WorkbooksRuntime.Plugin.Auth=) — three-context
     fetch (env / work env keychain / broker). ~60 LOC.
