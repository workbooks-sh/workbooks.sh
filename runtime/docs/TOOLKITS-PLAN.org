...
authoritative current shape. Deltas:
  ...
  - The `work toolkit` runner described here was Rust (cli/wb/.../toolkit.rs);
    in the clean-room it is reimplemented as Elixir subcommands in
    runtime/host/cli.ex (see TOOLKITS-V3.org §"work toolkit surface").
...
** 1. Pre/post-condition source blocks
   ...
   #+begin_src org
   ,* Workflow
     ,#+CAPTION: verify work is installed at the right version
     ,#+begin_src bash :role pre
     wb --version | grep -E '^wb 0\.[0-9]+\.' || { echo "work missing"; exit 1; }
     ,#+end_src
   ...
   The agent (or the eval runner, or =work toolkit check=) runs
...
** 2. Property-drawer declarations the runtime acts on
   ...
   These are first-class. The =work toolkit check= linter validates
...
** 3. OQL-queryable structure
   ...
   The =work toolkit search= helper is literally:

   #+begin_src bash
   work query '(and (tags video) (not (todo deprecated)))' \
...
** 4. Cross-skill links
   ...
   grepping. =work toolkit show <toolkit>= can render the link
...
** 5. Org-babel blocks the agent runs directly
   ...
   ,#+begin_src bash
   work init "$1" --template=document --no-install
   ,#+end_src

   ,#+CAPTION: install deps + first build
   ,#+begin_src bash
   cd "$1" && npm install --no-audit --no-fund && work build
   ,#+end_src
   #+end_src
...
** What the runtime owes
   ...
   - Parse :role pre/post blocks and expose them to =work toolkit
...
** Tier 1 — git URL (ship today)
   ...
   #+begin_src bash
   work toolkit install https://github.com/alice/wb-toolkit-fly
   # = shallow clone into ~/Workbooks/Workbox/toolkits/fly/
   #+end_src

   - Auth: relies on git's existing auth (SSH keys, etc.)
   - Updates: =work toolkit update fly= = =git pull= in the dir
   - Uninstall: =work toolkit remove fly= = =rm -rf=
...
** Tier 3 — Radicle / federation (ship when federation is live)
   ...
   =work toolkit install rad://<rid>= pulls via radicle-node. Lets
...
** When the agent doesn't know which skill it needs: =work toolkit search=

   #+begin_src bash
   work toolkit search "share workbook with password"
   # → fly/skills/...   (no hit)
...
* Reference implementation: =wb=
  ...
  Initial skills:
    ...
    - =scaffold-a-workbook.org= — work init + npm install + work build
    - =edit-and-rebuild.org= — unbundle, edit, rebuild
    - =share-a-workbook.org= — work publish + Live URL pattern
    - =seal-and-encrypt.org= — seal / unseal / encrypt for distribution
    - =query-across-workbooks.org= — =work workbooks= cross-workbook OQL

  The work toolkit is also the first thing the agent sees in any
...
** Phase 1 — convention + reference toolkit (wb-tlkt.1 .. .5)
   ...
   1. =wb-tlkt.1= — Author the 6 work skill files at =cli/wb/toolkit/=
      + manifest.org
   ...
   4. =wb-tlkt.4= — Eval: agent given vague workbook task,
      discovers work via index, reads scaffold skill, executes
      end-to-end; passes
...
** Phase 2 — authoring helpers (wb-tlkt.6 .. .9)
   ...
   1. =wb-tlkt.6= — =work toolkit list= command
   2. =wb-tlkt.7= — =work toolkit show <name>= command
   3. =wb-tlkt.8= — =work toolkit new <name> [--cli <bin>]= scaffold
   4. =wb-tlkt.9= — =work toolkit check <name>= linter

** Phase 3 — distribution (wb-tlkt.10 .. .12)

   1. =wb-tlkt.10= — =work toolkit install <git-url>= (Tier 1)
   2. =wb-tlkt.11= — =work toolkit search= cross-toolkit grep
   3. =wb-tlkt.12= — =work toolkit install @workbooks-toolkit/<name>=
      via npm (Tier 2)
...
* Success looks like
  ...
  shareable workbook with a password"/, accomplishes it by:
    ...
    2. =cat=ing =wb/skills/overview.org= → confirms work is the right
       toolkit
