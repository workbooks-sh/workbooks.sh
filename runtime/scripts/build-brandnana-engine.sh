...
# clean-room runtime AND monorepo trees (brandnana CLI + profile + wb + oql) — but
# the runtime lives under .claude/, which the root .dockerignore excludes. So we
...
rsync -a "${EX[@]}" "$ROOT/substrates/oql/" "$CTX/substrates/oql/"  # OQL crates (work dep)
rsync -a "${EX[@]}" "$ROOT/cli/wb/" "$CTX/cli/wb/"                  # work meta-CLI
