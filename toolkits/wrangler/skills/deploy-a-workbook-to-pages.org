...
#+CAPTION: Ship a built workbook .html to Pages on the user's own account, with the real wrangler CLI.
...
no =work forge web= command — that wrapper was removed with the Rust =wb=
