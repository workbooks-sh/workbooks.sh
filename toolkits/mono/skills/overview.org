...
* Workflow
  ...
  1. Build + register (once):
     ...
     #+CAPTION: Build the kernel in-sandbox and register it
     #+begin_src bash
  ...
  2. Loop it engine-side (the hot path):
     ...
     #+CAPTION: Open once, call per frame (Elixir, via Workbooks.Kernel)
     #+begin_src elixir
  ...
  3. Or one-shot from any client (desktop renderer, scripts):
     ...
     #+CAPTION: One frame over RCP (base64 in/out)
     #+begin_src bash
