#+TITLE: ffmpeg — frame rate · speed · timelapse · interpolation
#+TOOLKIT: ffmpeg
#+SKILL: video-fps-speed
#+VERSION: 0.1.0
#+DESCRIPTION: Use when you need to change a video's TEMPORAL properties — convert fps (30→24), speed it up or slow it down (slow-mo / fast-forward), build a timelapse, or interpolate to a higher fps. Covers the setpts-vs-fps distinction, keeping audio in sync, and minterpolate caveats.
#+TAGS: video fps framerate speed slowmo timelapse setpts minterpolate
#+STATUS: stable
...
* When to use this
  ...
  NOT for: trimming a range ([[file:trim.org][trim]]); resizing the frame
  ([[file:video-scale-crop.org][video-scale-crop]]).
...
** Convert frame rate (same duration)
  ...
  #+CAPTION: inspect the current frame rate
  ...
  #+CAPTION: retarget to 24 fps (drops/duplicates frames; duration unchanged)
  #+begin_src bash
  ...
  #+CAPTION: normalize a variable-frame-rate (VFR) source to constant 30
  #+begin_src bash
...
** Speed up / slow down (duration changes)
  ...
  #+CAPTION: 2× faster (video + pitch-preserved audio)
  #+begin_src bash
  ...
  #+CAPTION: 0.5× slow motion (video slowed; audio slowed to match)
  #+begin_src bash
  ffmpeg -i input.mp4 -filter_complex \
  ...
  #+CAPTION: 4× faster — chain atempo (each stage capped at 2.0)
  #+begin_src bash
  ffmpeg -i input.mp4 -filter_complex \
  ...
  #+CAPTION: speed up video, DROP audio entirely (silent fast-forward)
  #+begin_src bash
...
** Timelapse (collapse a long recording)
  ...
  #+CAPTION: extreme timelapse — keep 1 frame/sec, then play at 30fps
  #+begin_src bash
  ...
  #+CAPTION: build a timelapse FROM an image sequence (frame-####.jpg)
  #+begin_src bash
...
** Frame interpolation (minterpolate) — smooth fps boost / slow-mo
  ...
  #+CAPTION: interpolate 30fps → 60fps (motion-compensated)
  #+begin_src bash
  ...
  #+CAPTION: smooth slow-mo: interpolate up, THEN slow with setpts
  #+begin_src bash
  ffmpeg -i input.mp4 -filter_complex \
...
** Verify duration + frame rate
  ...
  #+CAPTION: confirm the timing change took
...
* See also
  ...
  - [[file:trim.org][trim]] — cut the range before retiming
  - [[file:video-audio-mux.org][video-audio-mux]] — deeper on audio sync (-shortest, async)
  - [[file:gif-webm-export.org][gif-webm-export]] — fps as a GIF size lever
  - [[file:lossy-tradeoffs.org][lossy-tradeoffs]] — preset cost matters a lot for minterpolate
