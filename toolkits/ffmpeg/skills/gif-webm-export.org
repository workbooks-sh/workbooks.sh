#+TITLE: ffmpeg — GIF · WebM/VP9 · animated WebP export
#+TOOLKIT: ffmpeg
#+SKILL: gif-webm-export
#+VERSION: 0.1.0
#+DESCRIPTION: Use when a clip needs to ship as an animated GIF, a WebM/VP9 video, or an animated WebP — for web embeds, ad previews, README demos, or autoplay loops. Covers the palettegen/paletteuse two-pass that fixes GIF banding, fps/scale size tradeoffs, and the "huge file" traps.
#+TAGS: gif webm vp9 webp animation loop palette export web ads
#+STATUS: stable
...
* When to use this
  ...
  NOT for: full-length deliverables (use [[file:transcode-to-h264.org][transcode-to-h264]]);
  a single still ([[file:extract-thumbnail.org][extract-thumbnail]] / [[file:image-ops.org][image-ops]]).

  The single biggest lever is *trim first* — a GIF's size scales
  with frames × resolution. Cut to the exact loop ([[file:trim.org][trim]]) BEFORE
  exporting.
...
** High-quality GIF (palettegen / paletteuse two-pass)
  ...
  #+CAPTION: verify input + decide the loop length
  ...
  #+CAPTION: pass 1 — build a content-optimal palette
  #+begin_src bash
  ffmpeg -i input.mp4 \
  ...
  #+CAPTION: pass 2 — render the GIF against that palette
  #+begin_src bash
  ...
  #+CAPTION: one-liner (palette stays in memory, no temp file)
  #+begin_src bash
...
** WebM / VP9 (the small, modern default)
  ...
  #+CAPTION: VP9 + Opus, CRF mode (quality-targeted, much smaller than GIF)
  #+begin_src bash
  ffmpeg -i input.mp4 \
  ...
  #+CAPTION: silent autoplay loop for web (no audio, faststart-style)
  #+begin_src bash
  ffmpeg -i input.mp4 \
  ...
  #+CAPTION: VP9 WITH alpha/transparency (yuva420p, for compositing)
  #+begin_src bash
...
** Animated WebP (GIF replacement that's still an <img>)
  ...
  #+CAPTION: animated WebP loop, quality + compression tuned
  #+begin_src bash
  ffmpeg -i input.mp4 \
...
** Verify size + that it actually animates
  ...
  #+CAPTION: confirm frame count > 1 and report size
...
* See also
  ...
  - [[file:trim.org][trim]] — cut to the exact loop BEFORE exporting (biggest size lever)
  - [[file:video-scale-crop.org][video-scale-crop]] — reframe/resize before the export
  - [[file:video-fps-speed.org][video-fps-speed]] — speed-ramp or timelapse the loop first
  - [[file:lossy-tradeoffs.org][lossy-tradeoffs]] — CRF mental model (applies to VP9 too)
