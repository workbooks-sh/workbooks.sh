#+TITLE: ffmpeg — trim a video (cut to a time range)
#+TOOLKIT: ffmpeg
#+SKILL: trim
#+VERSION: 0.1.0
#+DESCRIPTION: Use when you have a video and need a slice — first N seconds, between two timestamps, or remove the intro/outro. Stream-copy fast path vs re-encode precise path.
#+TAGS: video trim cut slice clip
#+STATUS: stable
...
* When to use this
  ...
  NOT for: concatenating multiple clips (=[[file:concat.org][concat]]=);
  re-encoding without trimming ([[file:transcode-to-h264.org][transcode-to-h264]]).

* Workflow

** Fast (stream-copy) — first 30 seconds

  #+CAPTION: verify input
  #+begin_src bash :role pre
  test -f "$1" || { echo "input file $1 not found"; exit 1; }
  ffprobe -v error -show_entries format=duration -of csv=p=0 "$1"
  #+end_src

  #+CAPTION: keyframe-aligned trim, no re-encode (very fast)
  #+begin_src bash
...
** Fast — between two timestamps
  ...
  #+CAPTION: clip from 1m15s to 2m00s without re-encoding
  #+begin_src bash
...
** Precise (re-encode) — frame-accurate cut
  ...
  #+CAPTION: precise — -ss AFTER -i decodes every frame up to T (slow but exact)
  #+begin_src bash
  ...
  #+CAPTION: confirm the output duration matches the cut
  #+begin_src bash :role post :path clip.mp4
...
* Common pitfalls
  ...
  5. *Re-encoding without =-pix_fmt yuv420p=.* Same gotcha as
     [[file:transcode-to-h264.org][transcode-to-h264]] — some pixel formats won't play in Safari/iOS.
...
* See also
  ...
  - [[file:concat.org][concat]] — stitch multiple trims together
  - [[file:transcode-to-h264.org][transcode-to-h264]] — re-encode without trimming
