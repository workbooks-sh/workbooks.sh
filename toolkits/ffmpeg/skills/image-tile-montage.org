#+TITLE: ffmpeg — tile & slice images (grid montage, contact sheet, vertical chunks)
#+TOOLKIT: ffmpeg
#+SKILL: image-tile-montage
#+VERSION: 0.1.0
#+DESCRIPTION: Use to combine N images into one grid (the tile filter), build a contact sheet, OR do the reverse — slice one tall image into N vertical chunks. Replaces the hand-rolled python png-slice loop with exact ffmpeg crop-per-chunk recipes.
#+TAGS: image tile montage grid contact-sheet slice chunk hstack vstack
#+STATUS: stable
...
* When to use this
  ...
  ([[file:image-overlay-composite.org][image-overlay-composite]]); a single resize/crop
  ([[file:image-resize.org][image-resize]] / [[file:image-crop-pad.org][image-crop-pad]]).
...
** From explicit inputs (small, fixed set)
  ...
  #+CAPTION: 2×2 grid, 10px outer margin, 5px gaps, white background
  #+begin_src bash
...
** From a directory of images (contact sheet)
  ...
  #+CAPTION: contact sheet — thumbnail to 200px wide, 4 across
  #+begin_src bash
  ...
  #+CAPTION: confirm the grid wrote + is a real image of the expected size
  #+begin_src bash :role post :path sheet.png
...
** Side-by-side / stacked via hstack / vstack (different sizes OK-ish)
  ...
  #+CAPTION: before/after side by side (normalize height first)
  #+begin_src bash
  ffmpeg -i before.png -i after.png \
    -filter_complex "[0:v]scale=-2:600[l];[1:v]scale=-2:600[r];[l][r]hstack" compare.png
  #+end_src

  #+CAPTION: 4-up grid via stacks (full manual control)
  #+begin_src bash
  ffmpeg -i a.png -i b.png -i c.png -i d.png \
...
* Slice — cut one TALL image into N vertical chunks
  ...
  #+CAPTION: split a tall PNG into N stacked chunks (chunk-00.png …)
  #+begin_src bash
  IN=tall.png; N=4
  ...
  #+CAPTION: fixed 1200px-tall chunks (count derived from height)
  #+begin_src bash
  IN=tall.png; CH=1200
  read -r W H < <(ffprobe -v error -show_entries stream=width,height -of csv=p=0 "$IN" | tr ',' ' ')
  ...
  #+CAPTION: confirm every chunk wrote + is a real image
...
* See also
  ...
  - [[file:image-overlay-composite.org][image-overlay-composite]] — layer images WITH transparency (not a grid)
  - [[file:image-crop-pad.org][image-crop-pad]] — the crop primitive the slice loop uses
  - [[file:image-resize.org][image-resize]] — the scale step in contact sheets
