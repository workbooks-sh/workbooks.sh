#+TITLE: ffmpeg — extract a thumbnail (single still frame)
#+TOOLKIT: ffmpeg
#+SKILL: extract-thumbnail
#+TAGS: video thumbnail still frame jpeg png
#+STATUS: stable
...
* Workflow
  ...
  #+CAPTION: 1. verify input exists + show its duration
  #+begin_src bash :role pre
  test -f "$1" || { echo "input file $1 not found"; exit 1; }
  ffprobe -v error -show_entries format=duration -of csv=p=0 "$1"
  #+end_src

  #+CAPTION: 2. extract a single frame at 2s
  #+begin_src bash
  ...
  #+CAPTION: 3. with explicit resize (e.g. 320x180)
  #+begin_src bash
  ffmpeg -ss 2 -i input.mp4 -frames:v 1 -vf scale=320:180 thumbnail.jpg
  #+end_src

  #+CAPTION: 4. webp for smaller size at comparable quality
  #+begin_src bash
  ffmpeg -ss 2 -i input.mp4 -frames:v 1 -q:v 80 thumbnail.webp
  #+end_src

  #+CAPTION: confirm output is a real image
  #+begin_src bash :role post :path thumbnail.jpg
...
* See also
  ...
  - [[file:transcode-to-h264.org][transcode-to-h264]] for full video re-encode
