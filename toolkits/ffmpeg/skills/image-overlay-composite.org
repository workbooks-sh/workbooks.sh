#+TITLE: ffmpeg — overlay & composite (watermark, logo, alpha blend)
#+TOOLKIT: ffmpeg
#+SKILL: image-overlay-composite
#+VERSION: 0.1.0
#+DESCRIPTION: Use to place one image on top of another — logo/watermark in a corner, centered badge, alpha-blended stamp. Covers overlay=x:y with main_w/overlay_w positioning math, scale-then-overlay, semi-transparent watermarks, and the input-order gotcha (base image FIRST).
#+TAGS: image overlay composite watermark logo alpha blend badge corner
#+STATUS: stable
...
* When to use this
  ...
  ([[file:image-tile-montage.org][image-tile-montage]] — that's =tile=/=hstack=, no
...
* Alpha blend — semi-transparent watermark
  ...
  #+CAPTION: 40%-opacity watermark, bottom-right
  #+begin_src bash
...
* Scale-then-overlay (size the logo relative to the base)
  ...
  #+CAPTION: fixed logo size (120px wide, keep aspect)
  #+begin_src bash
  ffmpeg -i base.png -i logo.png \
    -filter_complex "[1:v]scale=120:-1[wm];[0:v][wm]overlay=W-w-20:H-h-20" out.png
  #+end_src

  #+CAPTION: logo sized to a FRACTION of the base width (responsive)
  #+begin_src bash
  ffmpeg -i base.png -i logo.png \
  ...
  #+CAPTION: logo = 15% of the BASE width (resolve base dims first)
  #+begin_src bash
...
* The color-shift / format gotcha
  ...
  #+CAPTION: keep RGB/alpha through the composite (still images)
  #+begin_src bash
  ffmpeg -i base.png -i logo.png \
  ...
  [[file:image-convert.org][image-convert]].)
...
* See also
  ...
  - [[file:image-tile-montage.org][image-tile-montage]] — arrange images side-by-side (no layering)
  - [[file:image-convert.org][image-convert]] — final format + the JPG alpha-loss rule
  - [[file:image-resize.org][image-resize]] — scale the logo (or base) before compositing
  - [[file:image-ops.org][image-ops]] — quick-start index for all image transforms
  - [[file:subtitle-burn.org][subtitle-burn]] — the video analogue (burning text/graphics into frames)
