#+TITLE: ffmpeg — crop & pad an image (region, center, letterbox to aspect)
#+TOOLKIT: ffmpeg
#+SKILL: image-crop-pad
#+VERSION: 0.1.0
#+DESCRIPTION: Use when you need to cut a rectangular region out of an image (crop=w:h:x:y, centering math) OR add bars to fit a target aspect (pad / letterbox / pillarbox). Covers the crop-is-post-scale ordering gotcha and pad color.
#+TAGS: image crop pad letterbox pillarbox aspect center square
#+STATUS: stable
...
* When to use this
  ...
  ([[file:image-resize.org][image-resize]]); changing the FORMAT
  ([[file:image-convert.org][image-convert]]). Read current dims first:
...
* Crop — keep a rectangle
  ...
  #+CAPTION: verify input + read dims
  ...
  #+CAPTION: crop a 600×400 region whose top-left is (100,50)
  #+begin_src bash
...
** Centering math
  ...
  #+CAPTION: centered 600×400 crop (ffmpeg computes X,Y)
  #+begin_src bash
  ...
  #+CAPTION: explicit centered crop using iw/ih
  #+begin_src bash
...
** Center-crop to a square (= the shorter edge)
  ...
  #+CAPTION: square crop from the center, side = min(iw,ih)
  #+begin_src bash
...
** Trim a fixed margin off every edge
  ...
  #+CAPTION: shave 20px from all four sides
  #+begin_src bash
...
** Pad to an aspect ratio without a fixed pixel box
  ...
  #+CAPTION: pad height so the frame becomes ≥16:9 (centered, black)
  #+begin_src bash
  ffmpeg -i in.png -vf "pad='iw':'max(ih,iw*9/16)':(ow-iw)/2:(oh-ih)/2:black" out.png
  #+end_src

  #+CAPTION: pad width to make it 16:9 (pillarbox a tall image)
  #+begin_src bash
  ...
  #+CAPTION: confirm output dims + that it's a real image
...
* The crop-is-post-scale ordering gotcha
  ...
  #+CAPTION: scale to 1000 wide, THEN crop a 1000×600 band from y=200 (scaled space)
  #+begin_src bash
  ffmpeg -i in.png -vf "scale=1000:-2,crop=1000:600:0:200" out.png
  #+end_src

  #+CAPTION: crop the original region FIRST, then downscale the cutout
  #+begin_src bash
...
* Color for pad
  ...
  see [[file:image-convert.org][image-convert]] for the alpha-loss rule.
...
* See also
  ...
  - [[file:image-resize.org][image-resize]] — scale/downscale (the step that usually precedes pad)
  - [[file:image-convert.org][image-convert]] — format change + the alpha-loss rule for JPG
  - [[file:image-overlay-composite.org][image-overlay-composite]] — placing one image on top of another
