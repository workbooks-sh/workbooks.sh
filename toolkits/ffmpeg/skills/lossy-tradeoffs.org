#+TITLE: ffmpeg — lossy tradeoffs (CRF / bitrate / preset)
#+TOOLKIT: ffmpeg
#+SKILL: lossy-tradeoffs
#+VERSION: 0.1.0
#+DESCRIPTION: Mental model for picking CRF, bitrate, preset, and codec for a re-encode — what costs what, what to escalate when output looks/sizes wrong.
#+TAGS: video encoding quality bitrate crf preset h264 h265
#+STATUS: stable
...
** "File too big"
  ...
  #+CAPTION: try a quick re-encode at crf+4
  #+begin_src bash
...
** "Encode is way too slow"
  ...
  hardware encoder (see [[file:hardware-acceleration.org][hardware-acceleration]]).
...
** "Exact target size needed"
  ...
  #+CAPTION: two-pass for predictable size
  #+begin_src bash
...
* Common pitfalls
  ...
  4. *Preset on HW encoders maps differently.* NVENC's =-preset=
     ...
     [[file:hardware-acceleration.org][hardware-acceleration]] for the HW-specific knobs.
...
* See also
  ...
  - [[file:transcode-to-h264.org][transcode-to-h264]] — the baseline re-encode pattern
  - [[file:hardware-acceleration.org][hardware-acceleration]] — speed when CPU isn't enough
