#+TITLE: ffmpeg — concatenate multiple videos into one
#+TOOLKIT: ffmpeg
#+SKILL: concat
#+VERSION: 0.1.0
#+DESCRIPTION: Use when you have multiple video files (clips, episodes, splits) and need a single output. The right concat method depends on whether the inputs share codec / dimensions / framerate.
#+TAGS: video concat join stitch merge
#+STATUS: stable
...
** Fast path — concat demuxer (stream-copy)
  ...
  #+CAPTION: 1. confirm inputs are codec-compatible
  ...
  #+CAPTION: 2. write the concat list file
  #+begin_src bash
  cat > concat.txt <<EOF
  file 'clip1.mp4'
  file 'clip2.mp4'
  ...
  #+CAPTION: 3. concat without re-encoding (very fast)
  #+begin_src bash
...
** Slow path — concat filter (re-encode)
  ...
  #+CAPTION: concat 3 disparate clips, re-encoding to a uniform output
  #+begin_src bash
  ffmpeg \
...
** Confirm output
  ...
  #+CAPTION: total duration should ≈ sum of input durations
...
* See also
  ...
  - [[file:trim.org][trim]] — produce the inputs to concat
  - [[file:transcode-to-h264.org][transcode-to-h264]] — normalize inputs before concat-demuxer
