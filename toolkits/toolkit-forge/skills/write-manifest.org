#+TITLE: toolkit-forge — write-manifest (the index that routes by need)
#+TOOLKIT: toolkit-forge
#+SKILL: write-manifest
#+VERSION: 0.1.0
#+DESCRIPTION: Use when all skills are written — author the manifest.org with the canonical frontmatter, the :toolkit:-tagged node, and the skill-index table that lists every skill once, keyed by need.
#+TAGS: manifest index frontmatter toolkit-node skill-index drawer keywords
#+STATUS: experimental
...
* When to use this
  ...
  NOT for: writing skills ([[file:author-deep-skill.org][author-deep-skill]]); the final checks
  ([[file:verify-toolkit.org][verify-toolkit]]). Full spec: [[file:../../AUTHORING.org][AUTHORING.org §"manifest.org"]].
...
** The :toolkit:-tagged node + skill-index table
  ...
  #+CAPTION: the toolkit headline with the runtime-honored drawer + index
  #+begin_src org
...
** Confirm the index matches disk
  ...
  #+CAPTION: pre — the skills must exist before you index them
  #+begin_src bash :role pre
  ls toolkits/NAME/skills/*.org >/dev/null 2>&1 || { echo "no skills to index"; exit 1; }
  #+end_src

  #+CAPTION: post — every skill file appears in the table exactly once
...
* See also
  ...
  - [[file:author-deep-skill.org][author-deep-skill]] — write the skills this indexes
  - [[file:verify-toolkit.org][verify-toolkit]] — next: the whole-toolkit done-bar
  - [[file:../../AUTHORING.org][toolkits/AUTHORING.org]] — the manifest spec
