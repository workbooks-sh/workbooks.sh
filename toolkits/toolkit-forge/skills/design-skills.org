#+TITLE: toolkit-forge — design-skills (the need→skill progressive-disclosure map)
#+TOOLKIT: toolkit-forge
#+SKILL: design-skills
#+VERSION: 0.1.0
#+DESCRIPTION: Use after studying the surface — decide the toolkit name + CLI_BIN and partition the needs into the deep-skill set that the manifest will index by need.
#+TAGS: design partition needs skills progressive-disclosure naming cli-bin map
#+STATUS: experimental
...
* When to use this
  ...
  Phase 3 of the forge. You have the surface map ([[file:study-surface.org][study-surface]]).
  ...
  NOT for: writing the skills ([[file:author-deep-skill.org][author-deep-skill]]) or the manifest
  ([[file:write-manifest.org][write-manifest]]); those realize this design.

* The mental model: one need with its own gotchas = one skill

  From [[file:../../AUTHORING.org][AUTHORING.org §"Progressive disclosure"]]: the manifest is the
...
** Partition the needs into skills
  ...
  #+CAPTION: sanity — the needs you mapped should each land in exactly one skill
  #+begin_src bash
...
** Group + bundle
  ...
  #+CAPTION: the design is sound when the map round-trips
...
* See also
  ...
  - [[file:study-surface.org][study-surface]] — previous: the needs list this consumes
  - [[file:author-deep-skill.org][author-deep-skill]] — next: realize each skill in the map
  - [[file:write-manifest.org][write-manifest]] — build the index from this map
  - [[file:../../AUTHORING.org][toolkits/AUTHORING.org]] — the partition + disclosure rules
