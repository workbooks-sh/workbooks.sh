#+TITLE: toolkit-forge — overview (the forge loop + the standard)
#+TOOLKIT: toolkit-forge
#+SKILL: overview
#+VERSION: 0.1.0
#+DESCRIPTION: Use when first reaching for toolkit-forge — the four-phase loop (research → design → author → verify), where the standard lives, and which skill owns each step.
#+TAGS: overview forge toolkit authoring progressive-disclosure index
#+STATUS: experimental
...
* When to use this
  ...
  themselves, read [[file:../../AUTHORING.org][toolkits/AUTHORING.org]] — this toolkit OPERATES that
...
* The mental model: the substrate is the catalog
  ...
  [[file:../../AUTHORING.org][AUTHORING.org]]):
...
* The forge loop (four phases, one skill each)
  ...
  1. *Research* — resolve the target + fetch its source into a scratch
     dir. [[file:research-target.org][research-target]].
  2. *Study* — read the REAL =--help= / README / source to map the
     commands, the gotchas, and the distinct NEEDS (not the flags).
     [[file:study-surface.org][study-surface]].
  3. *Design* — pick the toolkit name + CLI_BIN, and the
     need→skill disclosure map (which deep skills exist).
     [[file:design-skills.org][design-skills]].
  4. *Author* — write each DEEP skill ([[file:author-deep-skill.org][author-deep-skill]]) + the
     [[file:write-manifest.org][manifest]] index.
  5. *Verify* — org balance, links resolve, recipes run where
     possible, manifest indexes every skill. [[file:verify-toolkit.org][verify-toolkit]].

  #+CAPTION: confirm the tools the forge needs are present
...
* Common pitfalls
  ...
  2. *Shallow skills.* Hitting the section headers but writing
     ...
     Depth is the bar — see [[file:author-deep-skill.org][author-deep-skill]].
  ...
  4. *Skipping verify.* Unbalanced =begin_src=, broken see-also links,
     ...
     [[file:verify-toolkit.org][verify-toolkit]].
...
* Verification checklist
  ...
  - [ ] You've read [[file:../../AUTHORING.org][AUTHORING.org]] before authoring
  - [ ] You'll run the phases in order (research → … → verify)
  - [ ] Output lands at =toolkits/<name>/= with manifest + skills

* See also

  - [[file:../../AUTHORING.org][toolkits/AUTHORING.org]] — the depth/breadth standard this operates
  - [[file:research-target.org][research-target]] — phase 1, fetch the target
  - [[file:verify-toolkit.org][verify-toolkit]] — phase 5, the done-bar
