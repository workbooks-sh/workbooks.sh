...
* Agent side (bash-only: poll the review endpoint)
  ...
  NEXT (SDK): a =wb ctk await <run>= subcommand wrapping this poll + a toolkit
  skill teaching the open→await→commit shape.
...
* Status / next
  ...
  + =GET /api/ctk/review/:id=, delivering into =Workbooks.AgentSession=; =mix
  compile= clean). NEXT: a =wb ctk await <run>= SDK subcommand + a toolkit skill
  for the agent's open→await→commit loop, persisting the approved snapshot, and a
  live round-trip test. Mark =stable= once a real run commits an approved change.
