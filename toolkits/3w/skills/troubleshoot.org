#+TITLE: 3w — troubleshoot (empty, walled, CAPTCHA'd, shell renders)
#+TOOLKIT: 3w
#+SKILL: troubleshoot
#+VERSION: 0.1.0
#+DESCRIPTION: Use when a 3w search or read came back empty, blocked, CAPTCHA'd, or rendered a JS shell. The bot-wall / empty-render playbook, seeded from spike findings.
#+TAGS: troubleshoot captcha bot wall blocked empty error proxy render fingerprint debug
#+STATUS: experimental
...
* When to use this
  ...
  routing around it.
  NOT for: normal usage ([[file:search.org][search]] /
...
** Search returned nothing
   #+CAPTION: pre — 3w available
   #+begin_src bash :role pre
   command -v 3w >/dev/null || { echo "3w not on PATH"; exit 1; }
   #+end_src

   #+CAPTION: widen the engine pool + simplify the query
   #+begin_src bash
...
** Read returned a JS shell / near-empty markdown
   #+CAPTION: force a real render with a longer wait
   #+begin_src bash
...
** 403 / 429 / CAPTCHA on a read
   #+CAPTION: route through a residential proxy (IP reputation is often the real signal)
   #+begin_src bash
...
* See also
  ...
  - [[file:search.org][search]] — normal search
  - [[file:read-extract.org][read-extract]] — normal reads + the tier model
  - [[file:overview.org][overview]] — the rules that avoid walls in the first place
