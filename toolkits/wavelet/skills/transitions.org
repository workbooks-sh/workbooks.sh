#+TITLE: wavelet — transitions between sections
#+TOOLKIT: wavelet
#+SKILL: transitions
#+TAGS: video transitions css keyframes
#+STATUS: experimental
...
* Cross-fade between two sections in ONE composition
  ...
  #+CAPTION: A holds 0..2.5s, cross-fades to B at 2.5..3.0s, B holds to end
  #+begin_src bash
  cat > seq.html <<'HTML'
  <!doctype html>
  <html><head><style>
...
* Verify
  ...
  #+begin_src bash :role post :path seq.mp4
...
* See also
  ...
  - [[file:text-cards.org][text-cards]] — the cards you transition between
