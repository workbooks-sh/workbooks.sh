...
#+TOOLKIT: palette
#+SKILL: zig
#+TAGS: zig wasm wasi compile authoring sandbox
#+STATUS: experimental

* The honest Zig story (engine = wasmtime only; see [[runtime-engine-wasmtime-only]])
...
* Author a command in Zig (the zigbuild path)
  ...
  #+CAPTION: native zig compiles the source → wasm32-wasi → registered command
  #+begin_src bash
