#+TITLE: palette — Node.js / npm
#+TOOLKIT: palette
#+SKILL: node
#+TAGS: node nodejs npm javascript bundle qjs wasm sandbox
#+STATUS: experimental

* The honest Node story (engine = wasmtime only; see [[runtime-engine-wasmtime-only]])
  ...
  3. *Full Node (node:fs / node:http / native addons)* → NOT available on wasmtime.
     ...
     deliberately declined ([[runtime-engine-wasmtime-only]]). The wasmtime-native
...
* Run npm code in the sandbox (the proven path)
  ...
  #+CAPTION: bundle an npm-using script to one file
  #+begin_src bash
  bun add <pkg> && bun build app.js --bundle --format=esm --outfile=bundle.js
  #+end_src

  #+CAPTION: run the bundle on the qjs runtime (sandboxed; grant the dir)
  #+begin_src bash
  qjs /w/bundle.js
  #+end_src
