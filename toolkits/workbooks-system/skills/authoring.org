...
Edit *source*, never the runnable artifact. A workbook ships as compiled
HTML with embedded =.wasm= / bundles; that file is an output. If you receive
a workbook as a single file and must change it: unbundle it to source, edit
the source, rebuild. Never patch compiled HTML, and never let raw build
inputs (un-compiled sources) leak into the shipped artifact — only WASM or
built bundles belong in it.
...
wb pack <workspace> <out>   # compose members into one workbook (--build to build first)
wb unpack <bundle> <dest>   # disassemble a workbook back into a flat source tree
