#+TITLE: open-avatars — integrate
#+TOOLKIT: open-avatars
#+SKILL: integrate
#+TAGS: integrate embed browser node wasm framework
#+STATUS: experimental
...
- =avatar(pack, seed, opts) -> string= — a complete =<svg>= document.
  - =pack= : a bundle object, or =null= to use a =register()='d default.
  - =seed= : any string. Hashed to pick atoms. Same seed → identical svg.
  - =opts= :
...
- =pick(pack, seed) -> { <cat>: <name> }= — which atom each category resolves to.
  Useful for debugging or showing the composition.
- =register(pack) -> void= — set a default bundle so you can call =avatar(null, …)=.
