<script lang="ts">
  ...
  const tooltip = $derived.by(() => {
    if (subject === "person") {
      if (state === "verified") {
        return realName
          ? `Verified identity — @${by ?? "this person"} signed in with WorkOS and "${realName}" is on file as their real name. Everything they publish is signed by their identity key.`
          : `Verified handle — @${by ?? "this person"}'s identity key has been confirmed. No real name on file, but signatures match.`;
      }
      return `Unverified — @${by ?? "this person"} hasn't bound their handle to a real-name service like WorkOS. Their posts are still signed cryptographically; we just haven't confirmed who they are off-platform.`;
    }
    // artifact
    if (state === "verified") {
