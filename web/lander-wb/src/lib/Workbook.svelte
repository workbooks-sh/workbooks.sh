<script>
  let { name, size, html, accent = "green", caption = "" } = $props();
</script>
<figure class="wb {accent}">
  <div class="bar"><span class="n mono">{name}</span><span class="s mono">{size} · one file</span></div>
  <iframe title={name} srcdoc={html} loading="lazy"></iframe>
