...
#+DESCRIPTION: Point work at a running runtime — local discovery by default, or a remote engine via one env var.
...
floor. The goal here: get =work= pointed at the right engine.
...
1. *Stand up / converge the engine.* =work deploy local= brings a runtime up in a
...
2. *Verify the connection.* Bare =work= (or =work info=) prints an environment +
...
work info                               # confirm it points at the remote
#+end_src
...
For ad-hoc RCP calls there's a raw escape hatch: =work rt status=, =work rt get <path>=,
=work rt post <path> [body]=. Useful for poking the runtime without an engine-backed
verb in front of it.
