...
=work= CLI — nothing else on your machine. The compilers, the model, and the
sandbox all live *inside* the runtime.

#+begin_quote
The CLI is =work=. Every command below is real and pinned to where it lives in the
...
One line installs the =work= binary. The toolchains it needs are *not* installed
...
If =work --help= prints the verb list, you're ready.
...
The agent thinks, compiles, and runs inside a *runtime engine*. =work deploy local=
...
=work deploy local= is the shorthand: it writes a =deployment.org= if absent, then
...
#+begin_src bash
work deploy secrets set OPENROUTER_API_KEY=sk-or-...
work deploy apply        # re-converge so the runtime picks it up
#+end_src

The key is stored under the name =OPENROUTER_API_KEY=; =work deploy secrets list=
...
:CAVEAT:   `work agent run` ships and reaches the model today; the agent writing a
...
Describe the workbook in plain English. =work agent run= starts a long-horizon run
on the runtime and returns an *id* you poll — the agent works in the background.

#+begin_src bash
work agent run "Make a workbook that shows the current weather \
...
#+begin_src bash
work agent status run_a1b2c3
#+end_src

If the draft isn't quite right, tell the agent again in the same plain-English
way with another =work agent run= — directing it is the loop. When you're happy,
...
#+begin_src bash
work workbook deploy weather weather.org
# → deployed weather.org → /w/weather
work workbook list                         # see it among your deployed workbooks
#+end_src

=work workbook deploy <id> <file>= reads the org source and =PUT=s it to =/w/<id>=
...
- *Author one yourself* — scaffold a blank workbook with =work init= and edit the
