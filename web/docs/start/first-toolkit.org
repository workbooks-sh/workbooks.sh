...
#+begin_src bash
work toolkit build-inline rev js ./reverse.js   # hot-swapped in place
#+end_src
...
#+begin_src bash
work toolkit promote rev js ./reverse.js
...
#+begin_src bash
work pack rev rev.wbundle           # one portable .wbundle (HTML + the wasm)
# on another machine:
work install rev.wbundle            # verifies + registers; now `run-command rev` works
#+end_src
