#+TITLE: work deploy & the OCI image
#+DESCRIPTION: Reference for the Deploy Kit — `work deploy` verbs, deployment.org, the ONE runtime image, local vs cloud, and the OQL kernel WIT world.

* work deploy & the OCI image :reference:
  :PROPERTIES:
  :OWNER:     deploy-kit
...
(layer 2, built by CI) are NOT =work deploy=. See [[*Release layers (do not conflate)][Release layers]].
#+end_quote
...
| verb                    | does                                                                | src                              |
|-------------------------+---------------------------------------------------------------------+----------------------------------|
| =work deploy init [preset]= | scaffold =./deployment.org=; preset =local= \vert =cloud= (TTY picks)  | =cli/src/deploy/mod.rs:34,161=   |
| =work deploy validate=   | coherence check on =deployment.org= incl. declared secrets          | =cli/src/deploy/mod.rs:40,152=   |
| =work deploy apply=      | converge to declared state (local container or provider recipe)     | =cli/src/deploy/mod.rs:42,363=   |
| =work deploy status=     | inspect the live deployment                                         | =cli/src/deploy/mod.rs:44,98=    |
| =work deploy logs=       | tail the live deployment's logs                                    | =cli/src/deploy/mod.rs:46,99=    |
| =work deploy down=       | tear it down                                                        | =cli/src/deploy/mod.rs:48,100=   |
| =work deploy local=      | shorthand: scaffold local config if absent, then apply              | =cli/src/deploy/mod.rs:50,101=   |
| =work deploy doctor=     | engines, recipes, declared secrets — present vs missing             | =cli/src/deploy/mod.rs:52,527=   |
| =work deploy secrets …=  | manage kit-abstracted, provider-delivered secrets (subcommands)     | =cli/src/deploy/mod.rs:54,223=   |

Note: the binary is invoked =work= (=cli/src/main.rs:24=, =name = "work"=). Older
docs and source moduledoc comments still say =work deploy=; the shipped command is
=work deploy=.
...
| verb                                       | does                                                          | src                            |
|--------------------------------------------+--------------------------------------------------------------+--------------------------------|
| =work deploy secrets set KEY=VALUE …=       | stage KEY=VALUE pairs (or =--from-env <file>=)               | =cli/src/deploy/mod.rs:61,225= |
| =work deploy secrets list=                  | list secret NAMES (never values)                             | =cli/src/deploy/mod.rs:65,251= |
| =work deploy secrets unset KEY=             | remove a secret by name                                      | =cli/src/deploy/mod.rs:67,258= |
| =work deploy secrets push=                  | push staged secrets to the provider without a deploy         | =cli/src/deploy/mod.rs:71,266= |
...
The shipped =work deploy= parses =deployment.org= as flat =#+KEYWORD:= lines
...
talk to it with =work rt status= (=cli/src/deploy/mod.rs:464=).
...
| 3 =work deploy=       | runs the runtime image (user's registry/host) | USERS only — init/validate/apply/…                    | =cli/src/deploy/mod.rs:82=       |

Layer-1/2 maintainer functions (NOT CLI verbs; not =work deploy=):
...
- Concepts: [[../concepts/the-dock.org][the Dock & capabilities]] · isolation tiers.
- Reference: [[./cli.org][work CLI]] · [[./caps.org][Dock capabilities]].
