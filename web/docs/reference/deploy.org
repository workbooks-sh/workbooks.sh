#+TITLE: wbx deploy & the OCI image
#+DESCRIPTION: Reference for the Deploy Kit — `wbx deploy` verbs, deployment.org, the ONE runtime image, local vs cloud, and the OQL kernel WIT world.

* wbx deploy & the OCI image :reference:
  :PROPERTIES:
  :OWNER:     deploy-kit
...
(layer 2, built by CI) are NOT =wbx deploy=. See [[*Release layers (do not conflate)][Release layers]].
#+end_quote
...
- Reference: [[./cli.org][wbx CLI]] · [[./caps.org][Dock capabilities]].
