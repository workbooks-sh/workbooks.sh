#+TITLE: Compilers (in-sandbox language lanes)
#+DESCRIPTION: Every compiler lane that turns source into a WASM command inside the sandbox — language, mechanism, status, limits.

* Compilers

The /compiler-in-WASM/ framework runs each language's compiler ITSELF as a wasm
...
| =js=     | JavaScript     | QuickJS-ng → wasm objects + work harness               | compile-to-wasm     | ships-today |
...
c4 (rswier/c4) — a real self-hosting C-/subset/ compiler+VM with its own codegen
...
built /for/ wasm32-wasi, so the compiler is itself a wasm module on wasmtime that
...
A wasm-hosted zig that emits wasm /directly/ (self-hosted wasm backend) is
...
QuickJS-ng (v0.10.0) compiled to wasm objects via the clang lane + the work harness
...
- [[../concepts/the-dock.org][The Dock & capabilities]] — what a compiled command may reach at runtime.
- [[./caps.org][Dock capabilities]] — the runtime capability surface.
