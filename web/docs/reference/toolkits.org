#+TITLE: Toolkits
#+DESCRIPTION: The toolkit authoring surface — manifest schema, EXEC shapes, capability grants, build lanes, and the work toolkit CLI. Tangled from code.
...
# ../concepts/what-is-a-toolkit.org → links to /learn. Do not narrate here.
...
Conceptual orientation (do not duplicate here): [[../concepts/what-is-a-toolkit.org][What is a toolkit]],
[[../concepts/exec-shapes.org][The six shapes]], [[../concepts/the-dock.org][The Dock & capabilities]].
...
Ships =:role task= bash blocks run via =work toolkit run <tk> <task> -- …=. Verify
...
(=toolkits.ex:507-521=). Recognized prefixes:
  ...
  :CAVEAT:   crate:/path:/wasm:/archive: build through `work toolkit build`; git+ is parsed but NOT yet supported by build (returns an error, toolkits.ex:891); gobuild:/script:/zigbuild: are parsed but script: is removed (wb-9ja).
  :END:
...
|------------------------------------------+-------------------------------------------------------------+-----------------|
| =work toolkit list=                       | list discoverable toolkits (id · status · tagline)          | l.327           |
| =work toolkit show <id> [skill]=          | manifest + skill index, or one skill's recipe               | l.328           |
| =work toolkit search <q>=                 | substring match across toolkits + skills                    | l.329           |
| =work toolkit verify <id>=                | structural + cap + trust + exec contract checks             | l.330           |
| =work toolkit sign <id>=                  | sign a manifest (=did:key= provenance; req. third-party)    | l.331           |
| =work toolkit build <id> [which]=         | in-sandbox compile + register the declared artifact         | l.332           |
| =work toolkit push <id> <dir>=            | ship a toolkit dir onto the engine (deploy-the-toolkit)     | l.333           |
| =work toolkit import <source> [--as] [-o]=| package a Claude skill / markdown / folder as a toolkit      | l.334           |
| =work toolkit audit <dir> [--fix]=        | re-run the wasm-compatibility audit on an imported dir       | l.335           |
| =work toolkit run <id> <task> -- <args>=  | run a =:role task= recipe (gated: =WB_TOOLKIT_EXEC=1=)       | l.336           |
...
=work toolkit show <id> <skill>=. The runtime resolves *which* toolkit; it never
inlines the manual.
...
— see [[../build/author-a-toolkit.org][Author a toolkit]] for the how-to.
...
- [[../concepts/what-is-a-toolkit.org][Concept: what is a toolkit]] → links to the /learn deep page.
- [[exec-shapes.org][Reference: #+EXEC shapes]] · [[caps.org][Reference: Dock capabilities]] · [[manifest.org][Reference: manifest fields]]
- [[../build/author-a-toolkit.org][How-to: author a toolkit]]
