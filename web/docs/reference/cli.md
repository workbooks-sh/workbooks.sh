...
> Every wbx verb, flag, mode, env var, and exit code — tangled from the clap surface.
...
> The binary is `wbx`, not `wb`. The legacy Elixir escript (`runtime/host/cli.ex`)
> and the dead npm `@work.books/cli` are superseded; this crate is canonical.
> :SRC: cli/Cargo.toml#bin (`name = "wbx"`, line 10) · cli/SPEC.md:6
...
- **ENGINE** — a thin RCP call into a running runtime (it owns the compilers,
  ...
  try =wbx deploy local=" hint when none is reachable.
...
| `wbx completions` | `shell` (bash\vbar zsh\vbar fish\vbar elvish\vbar powershell) | shell completions to stdout | LOCAL | :52, :295 |
...
| `wbx search` | `query` [`--semantic` \vbar `--literal` \vbar `--sql`] | cross-workbook search (default: `hybrid`) | ENGINE | :90, :321 |
...
| --- | --- | --- | --- |
| `wbx deploy init` | [`preset` local\vbar cloud] | scaffold `./deployment.org` (TTY picks if omitted) | :34, :84 |
...
> `wbx deploy` is the **user's** tool to run the runtime image for their own use. It
...
| verb | args / flags | does | Run | src (main.rs) |
| --- | --- | --- | --- | --- |
| `wbx rt` | `args…` (status\vbar get \pth\vbar post \pth [body]) | raw RCP escape hatch | ENGINE | :127, :361 |
| `wbx var` | `args…` (set\vbar get\vbar list\vbar ref) | local variables / secrets (ref-only) | LOCAL | :131, :363 |

# Not-yet-built: `wbx docs *`

- **MATURITY:** north-star
- **CAVEAT:** The `wbx docs` verb family (new/build/drift/lint/untangle/serve/publish)
...
- **MATURITY:** partial
- **CAVEAT:** runtime/host/cli.ex is the pre-wbx escript (~29 verbs / 67 forms,
- **SRC:** runtime/host/cli.ex · cli/SPEC.md:7-9 · cli/TAXONOMY.md:4

# See also

- Capability surface a toolkit can request: [Dock capabilities](caps.md)
- Toolkit EXEC shapes: [exec-shapes](exec-shapes.md) · manifest: [manifest](manifest.md)
- Why build/run delegate to the engine: [the Dock](../concepts/the-dock.md)
