...
token-authed verb subset of the control plane the thin =work= CLI drives
...
** RCP engine verbs for the =work= CLI (control plane, token-authed)
   :PROPERTIES:
   :MATURITY: ships-today
...
| method | path | work verb | does | :SRC: |
|--------+------+----------+------+-------|
| POST | =/rcp/build=             | =work build=     | compile a workspace's components → WASM      | =web.ex:440= |
| POST | =/rcp/library/checkout=  | =work checkout=  | borrow a library member (zip back to caller) | =web.ex:455= |
| POST | =/rcp/library/checkin=   | =work checkin=   | pack a working tree back (zip in)            | =web.ex:473= |
| GET  | =/rcp/store=             | =work store --list= | list stored archive keys                  | =web.ex:492= |
| POST | =/rcp/store=             | =work store=     | archive a workspace                          | =web.ex:498= |
| GET  | =/rcp/fetch=             | =work fetch=     | restore stored bytes (base64)                | =web.ex:656= |
...
- [[./cli.org][work CLI reference]] — the verbs that drive the RCP surface.
