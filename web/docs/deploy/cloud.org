...
#+begin_src bash
work deploy init cloud   # scaffold deployment.org for the cloud preset
work deploy secrets set KEY=VALUE
work deploy apply        # converge via the provider recipe
#+end_src
...
the =work= binary, materialized under the app dir (standalone installs).
...
=work deploy secrets=, and delivered per provider — staged through the recipe's
