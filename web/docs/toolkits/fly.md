...
> Deploy the Workbooks engine — or a forged app — onto a Fly.io machine on your own account, with just a flyctl login.

- **MATURITY:** partial
- **EVIDENCE:** toolkits/fly/deploy/bootstrap.sh:1
- **CAVEAT:** This is a `wb deploy` recipe (USER tool), not a registry toolkit you `wbx toolkit add`. It deploys the engine to a Fly machine via flyctl; it is not a general Fly API surface for arbitrary apps. See [deploy/cloud.md](../deploy/cloud.md) for the full deploy flow.
...
- **MATURITY:** partial
- **EVIDENCE:** toolkits/fly/deploy/bootstrap.sh:8
- **CAVEAT:** This runs through `wb deploy`, the consumer Deploy Kit — not `wbx toolkit add`. Do not confuse it with platform-release ops (the compilers package and runtime image ship separately).
...
> **Caution:** `wb deploy` is the **user's** tool to run the runtime image on their own account — it is not how the platform publishes the compilers package or the runtime image (those are a separate, manual/CI path). The Fly token is flyctl's own; Workbooks never reads or proxies it.
...
- Deploy how-to: [Deploy to the cloud](../deploy/cloud.md) · [The three release layers](../deploy/release-layers.md)
