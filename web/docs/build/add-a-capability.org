#+TITLE: Grant a capability to a toolkit
#+DESCRIPTION: Declare the caps a toolkit imports, match them to its WIT world, and pick the right isolation profile so the grant is real.
...
2. *Import it in the Dock SDK.* The cap list in =dock::bind!= (Rust) or =bind(imports)=
   ...
   See [[dock-sdk.org][The Dock SDK]].
   :PROPERTIES:
   :MATURITY:  ships-today
   :EVIDENCE:  web/docs/build/dock-sdk.org:13
   :END:
...
- Isolation profiles in depth: [[../run/tiers.org][Isolation tiers & trust]]
- Full cap reference: [[../reference/caps.org][Dock capabilities]]
