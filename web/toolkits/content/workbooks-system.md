...
#  - work available in-process; network only when granted, via a curl-like fetch
```

## What it grants

- `overview`, `concepts` (what *is* a workbook/runtime/toolkit/workflow/Dock), `authoring`, `toolkits`, `workflows`, and `writing` skills.
- The invariants an in-runtime agent must not break, and the line between in-runtime work and host/operator work (provisioning, image builds, `work deploy`, cloud lifecycle — out of scope).

## Maturity

Experimental (v0.1.0). `work` is host-provided — every Workbooks engine fulfills it in-process, tenant-scoped; the toolkit bundles no binary by design. If `work` is unavailable, you are not on a Workbooks runtime.
