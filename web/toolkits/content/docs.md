...
```
# author with the docs-authoring skill, then run the work docs verb family:
work docs new reference/my-api    # scaffold a page (north-star)
work docs build                   # render + assemble (render/deploy half is partial)
work docs drift                   # the keystone CI gate (north-star)
```

## What it grants

- A reusable Diátaxis IA over org source, reused — not reinvented — on top of `orgitorial` (renderer), `publish` (identity), and `work tangle`/`lint`.
