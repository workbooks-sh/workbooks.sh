# `work` CLI — canonical verb taxonomy (proposal)
...
### workbook (the file)
```
work build  [src]            L*  compile code components → WASM (package manager; resolves deps)
work bundle [src] [-o out]   L   assemble org + code + data → one workbook (.wbundle/.html)
work unbundle <file> [dest]  L   workbook → source tree            (was: unpack)
work run <workbook> [args]   L*  execute a workbook / component
work publish <workbook>      O   ship an assembled workbook to a surface (CF Pages/gh-pages/host)
...
```
work query <file.org>   K   headlines / structure rows
work plan  <file.org>   K   the build plan / WIT world        (was: tangle — see open Q)
work lint  <file.org>   K   diagnostics
```

### library (the tenant's many workbooks; remote)
```
work library                 R   list workspaces + members
work checkout <member> <dir> R   borrow a member into a working dir
work checkin  <member> <dir> R   pack + sign a member back
work search <query>          R   cross-workbook search  (--semantic | --literal | --sql)
work store  <slug>           R   archive a workspace to durable storage
work store --list                list stored                       (was: stored)
work fetch  <key> [out]      R   restore from storage
```

### runtime (the engine)
```
work deploy   init|validate|apply|status|logs|down|local|doctor   O   stand up the engine
work rt       status|get <path>|post <path> [json]                R   raw RCP escape hatch
work workflow run|plan <file.org>                                 R   run/plan a workflow DAG   [NEW]
work agent    run|status|logs <id>                                R   long-horizon agent runs    [NEW]
...
### identity & provenance
```
work sign   <file> [-o out]   L   embed a did:key provenance manifest
work verify <file>            L   check signature + integrity
work ledger <slug>            R   verify a run's signed ledger
work mirror <url> | --forge … O   mirror the tenant repo to a git host
work federate                O   federate over Radicle (P2P)        (was: radicle)
```

### config + system
```
work var set|get|list|ref     local variables / secrets (ref-only)
work telemetry [<slug>]       run index / one run's summary
work compiler list|build|run  in-sandbox compiler tooling (advanced)
work isolation                show the isolation-tier ladder (info)
work desktop install|open     the GUI app
work version
```
...
| `work query <org>` (kernel) vs `library query <sql>` | kernel stays **`query`**; library SQL folds into **`search --sql`** | the word "query" meant two things |
| `stored` | **`store --list`** | one storage verb family |
| `radicle` | **`federate`** | name the action, not the tool |
| `deploy build` / `deploy publish` (push runtime image to ghcr) | **maintainer/CI only**, not a user verb (platform release ≠ `work deploy`) | matches the 3-layer release rule |

## 6. Gaps filled (routes that had no verb)

The runtime exposes routes with no CLI today. New first-class verbs (thin RCP clients):
- **`work run`** — execute a workbook/component (was implicit).
- **`work workflow run|plan`** — `POST /api/workflow` (+`?plan=1`).
- **`work agent run|status|logs`** — `POST /api/run`, `GET /api/run/:id[/stream]` (was: hand-rolled `work rt post`).
- **`work workbook list|show|deploy`** — `GET /api/workbooks`, `/api/w/:id/org`, `PUT /w/:id`.
...
2. **`build`/`run` delegate to the runtime.** The compiler toolchain is the ~600 MB in-sandbox
   ...
   thin and delegates; with no server, the user gets a local engine via `work deploy local`
...
- **Engine-backed (delegate to a running runtime over RCP):** `build` `run` `library` `checkout`
  ...
  (engine-backed verbs error with "no runtime — try `work deploy local`" when none is reachable.)
