...
INJ=[max(2,int(x)) for x in torch.linspace(2,NL-3,4).tolist()]
...
with torch.no_grad():
  for w in range(NWIN):
    ...
    feats_c.append(torch.cat([hs[l] for l in INJ],dim=-1).squeeze(0).cpu())  # (CTX, H*4)
    blocks.append(blk)
feats_c=torch.stack(feats_c); blocks=torch.stack(blocks)
ntr=int(NWIN*0.9)
log(f"[t] cached {NWIN} windows in {time.time()-t0:.0f}s. training drafter (fast)…")
class D(nn.Module):
  def __init__(s):
    super().__init__(); s.inj=nn.Linear(H*len(INJ),H); s.fn=nn.RMSNorm(H)
...
def run(idx):
  cf=feats_c[idx].to(DEV); blk=blocks[idx].to(DEV)
  be=embed(torch.full((len(idx),N),MASK,device=DEV))*EMB
